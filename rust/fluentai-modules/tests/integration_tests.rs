...
use fluentai_modules::{ModuleLoader, ModuleConfig, ModuleEnvironment};
...
fn test_import_statement_parsing() {
    ...
    match root {
        fluentai_core::ast::Node::Import { module_path, import_list, import_all } => {
...
fn test_qualified_variable() {
    ...
    match root {
        fluentai_core::ast::Node::QualifiedVariable { module_name, variable_name } => {
...
fn test_import_all_syntax() {
    ...
    match root {
        fluentai_core::ast::Node::Import { module_path, import_all, .. } => {
            assert_eq!(module_path, "math");
            assert!(import_all);
        }
        _ => panic!("Expected Import node"),
...
fn test_nested_module_access() {
    ...
    let root = graph.get_node(graph.root_id.unwrap()).unwrap();
    match root {
        fluentai_core::ast::Node::QualifiedVariable { module_name, variable_name: _ } => {
