...
impl ModuleLoader {
        ...
        match result {
            Ok(module) => {
                ...
                if self.config.enable_cache {
                    ...
                    self.cache
                        .get(&module_id)
                        .ok_or_else(|| ModuleError::CacheError {
                            message: "Failed to retrieve module from cache after insertion"
                                .to_string(),
                        })
                } else {
                    Ok(Arc::new(module))
    ...
    fn validate_module_ref(&self, module_ref: &str) -> Result<()> {
        ...
        if module_ref.contains("..") || module_ref.contains("./") || module_ref.contains("/.") {
            warn!(
                "Rejected module reference with directory traversal: {}",
                module_ref
            );
            return Err(ModuleError::InvalidPath {
    ...
    fn validate_resolved_path(&self, path: PathBuf) -> Result<PathBuf> {
        ...
        if !is_within_search_path {
            warn!(
                "Module path {:?} is outside allowed search paths",
                canonical_path
            );
            return Err(ModuleError::InvalidPath {
                path: canonical_path,
    ...
    fn load_module_from_path(&mut self, path: &Path) -> Result<ModuleInfo> {
        ...
        let root = graph.root_id.unwrap_or_else(|| {
            ...
            graph
                .add_node(dummy)
                .expect("Failed to create dummy root node")
        });
...
mod tests {
    ...
    fn test_reject_directory_traversal() {
        ...
        for module_ref in dangerous_refs {
            ...
            assert!(
                matches!(result, Err(ModuleError::InvalidPath { .. })),
                "Should reject module ref: {}",
                module_ref
            );
        }
    }
    ...
    fn test_reject_absolute_paths() {
        ...
        #[cfg(unix)]
        let absolute_refs = vec!["/etc/passwd", "/usr/bin/evil", "/absolute/path/module.ai"];
