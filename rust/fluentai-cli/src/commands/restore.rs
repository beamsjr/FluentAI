...
pub async fn restore(
    ...
    for (i, package) in resolved.iter().enumerate() {
        ...
        lock_entries.insert(package.name.clone(), LockEntry {
...
async fn resolve_dependencies(deps: &[Dependency]) -> Result<Vec<ResolvedPackage>> {
    ...
    for dep in deps {
        resolved.push(ResolvedPackage {
            ...
            download_url: format!("{}/packages/{}/{}.aipkg", 
                                 DEFAULT_REGISTRY, 
                                 dep.name, 
                                 dep.version),
...
async fn download_package(package: &ResolvedPackage, packages_dir: &Path) -> Result<PathBuf> {
    let package_dir = packages_dir.join(&package.name).join(&package.resolved_version);
