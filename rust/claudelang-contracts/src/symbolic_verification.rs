...
impl SymbolicContractVerifier {
    ...
    fn find_function(&self, graph: &Graph, function_name: &str) -> ContractResult<NodeId> {
        ...
        for (_id, node) in &graph.nodes {
            if let claudelang_core::ast::Node::Application { function, args } = node {
                if let Some(claudelang_core::ast::Node::Variable { name }) = graph.get_node(*function) {
                    if name == "define" && args.len() == 2 {
                        if let Some(claudelang_core::ast::Node::Application { function: fname_id, .. }) = graph.get_node(args[0]) {
                            if let Some(claudelang_core::ast::Node::Variable { name: fname }) = graph.get_node(*fname_id) {
    ...
    fn extract_parameters(&self, graph: &Graph, function_name: &str) -> ContractResult<Vec<String>> {
        ...
        for (_id, node) in &graph.nodes {
            if let claudelang_core::ast::Node::Application { function, args } = node {
                if let Some(claudelang_core::ast::Node::Variable { name }) = graph.get_node(*function) {
                    if name == "define" && args.len() == 2 {
                        if let Some(claudelang_core::ast::Node::Application { function: fname_id, args: param_ids }) = graph.get_node(args[0]) {
                            if let Some(claudelang_core::ast::Node::Variable { name: fname }) = graph.get_node(*fname_id) {
                                if fname == function_name {
                                    let mut params = Vec::new();
                                    for param_id in param_ids {
                                        if let Some(claudelang_core::ast::Node::Variable { name }) = graph.get_node(*param_id) {
