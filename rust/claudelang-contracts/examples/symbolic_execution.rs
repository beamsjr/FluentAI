...
fn find_definition(graph: &claudelang_core::ast::Graph, function_name: &str) -> Option<claudelang_core::ast::NodeId> {
    use claudelang_core::ast::Node;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== Example 1: Absolute Value Function ===");
    {
        ...
        let contract = Contract {
            ...
            postconditions: vec![
                ContractCondition {
                    expression: graph.add_node(claudelang_core::ast::Node::Application {
                        function: graph.add_node(claudelang_core::ast::Node::Variable { name: ">=".to_string() } ),
                        args: vec![
                            graph.add_node(claudelang_core::ast::Node::Variable { name: "result".to_string() } ),
                            graph.add_node(claudelang_core::ast::Node::Literal(
    ...
    println!("\n\n=== Example 4: Contract Verification ===");
    {
        ...
        let contract = Contract {
            ...
            postconditions: vec![
                ContractCondition {
                    expression: graph.add_node(claudelang_core::ast::Node::Application {
                        function: graph.add_node(claudelang_core::ast::Node::Variable { name: "or".to_string() } ),
                        args: vec![
                            graph.add_node(claudelang_core::ast::Node::Application {
                                function: graph.add_node(claudelang_core::ast::Node::Variable { name: "=".to_string() } ),
                                args: vec![
                                    graph.add_node(claudelang_core::ast::Node::Variable { name: "y".to_string() } ),
                                    graph.add_node(claudelang_core::ast::Node::Literal(
                                        claudelang_core::ast::Literal::Integer(0)
                                    )),
                                ],
                            ...
                            graph.add_node(claudelang_core::ast::Node::Application {
                                ...
                                args: vec![
                                    ...
                                    graph.add_node(claudelang_core::ast::Node::Application {
                                        function: graph.add_node(claudelang_core::ast::Node::Variable { name: "/".to_string() } ),
                                        args: vec![
                                            graph.add_node(claudelang_core::ast::Node::Variable { name: "x".to_string() } ),
                                            graph.add_node(claudelang_core::ast::Node::Variable { name: "y".to_string() } ),
                                        ],
                                    }),
                                ],
