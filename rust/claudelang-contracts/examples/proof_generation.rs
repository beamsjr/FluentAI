...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== Example 1: Direct Proof for Absolute Value ===\n");
    {
        ...
        let ge_fn = graph.add_node(claudelang_core::ast::Node::Variable { name: ">=".to_string() });
        let result_var = graph.add_node(claudelang_core::ast::Node::Variable { name: "result".to_string() });
        let zero = graph.add_node(claudelang_core::ast::Node::Literal(
            claudelang_core::ast::Literal::Integer(0)
        ));
        
        let postcond_expr = graph.add_node(claudelang_core::ast::Node::Application {
            function: ge_fn,
    ...
    println!("\n=== Example 2: Induction Proof for Factorial ===\n");
    {
        ...
        let ge_fn = graph.add_node(claudelang_core::ast::Node::Variable { name: ">=".to_string() });
        let result_var = graph.add_node(claudelang_core::ast::Node::Variable { name: "result".to_string() });
        let one = graph.add_node(claudelang_core::ast::Node::Literal(
            claudelang_core::ast::Literal::Integer(1)
        ));
        ...
        // Precondition: n >= 0
        let n_var = graph.add_node(claudelang_core::ast::Node::Variable { name: "n".to_string() });
        let zero = graph.add_node(claudelang_core::ast::Node::Literal(
            claudelang_core::ast::Literal::Integer(0)
        ));
        let ge_fn2 = graph.add_node(claudelang_core::ast::Node::Variable { name: ">=".to_string() });
        
        let precond_expr = graph.add_node(claudelang_core::ast::Node::Application {
            function: ge_fn2,
            args: vec![n_var, zero],
        });
    ...
    println!("\n=== Example 5: Automated Proof ===\n");
    {
        ...
        let ge_fn1 = graph.add_node(claudelang_core::ast::Node::Variable { name: ">=".to_string() });
        let ge_fn2 = graph.add_node(claudelang_core::ast::Node::Variable { name: ">=".to_string() });
        let and_fn = graph.add_node(claudelang_core::ast::Node::Variable { name: "and".to_string() });
        let result_var1 = graph.add_node(claudelang_core::ast::Node::Variable { name: "result".to_string() });
        let result_var2 = graph.add_node(claudelang_core::ast::Node::Variable { name: "result".to_string() });
        let a_var = graph.add_node(claudelang_core::ast::Node::Variable { name: "a".to_string() });
        let b_var = graph.add_node(claudelang_core::ast::Node::Variable { name: "b".to_string() });
        
        let cond1 = graph.add_node(claudelang_core::ast::Node::Application {
            function: ge_fn1,
            args: vec![result_var1, a_var],
        });
        
        let cond2 = graph.add_node(claudelang_core::ast::Node::Application {
            function: ge_fn2,
