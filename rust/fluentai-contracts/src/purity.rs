...
impl<'a> PurityChecker<'a> {
    ...
    fn register_pure_builtins(&mut self) {
        ...
        self.pure_functions.extend(
            [
                "+", "-", "*", "/", "mod", "modulo", "%", "abs", "min", "max", "floor", "ceiling",
                "round", "sqrt", "pow",
            ]
            .iter()
            .map(|s| s.to_string()),
        ...
        self.pure_functions.extend(
            [
                "car", "cdr", "cons", "list", "length", "append", "reverse", "member", "filter",
                "map", "fold", "first", "rest", "nth", "take", "drop",
            ]
            .iter()
            .map(|s| s.to_string()),
        );
        ...
        self.pure_functions.extend(
            [
                "old", "result", "forall", "exists", "implies", "∀", "∃", "range", "in", "indices",
            ]
            .iter()
            .map(|s| s.to_string()),
        );
    ...
    fn check_node_purity(&mut self, node: &Node) -> ContractResult<bool> {
        match node {
            ...
            Node::Application { function, args } => self.check_application_purity(*function, args),
            ...
            // Note: FluentAi doesn't have a Sequence node type
            ...
            Node::Effect { effect_type, .. } => Ok(*effect_type == EffectType::Pure),
    ...
    fn check_application_purity(
        ...
        let func_node = self.graph.get_node(function_id).ok_or_else(|| {
            ContractError::VerificationError(format!("Function node {:?} not found", function_id))
        })?;
    ...
    pub fn validate_contract_purity(
        &mut self,
