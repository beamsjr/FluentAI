//! Convert FluentAi AST expressions to Z3 formulas

#[cfg(feature = "static")]
pub mod implementation {
    use crate::{
        errors::{ContractError, ContractResult},
        quantifiers::{Quantifier, QuantifierDomain, QuantifierParser},
    };
    ...
    /// Converts FluentAi AST expressions to Z3 formulas
    ...
    impl<'ctx> Z3Converter<'ctx> {
        ...
        /// Convert a FluentAi AST node to a Z3 expression
        pub fn convert_node(&self, node_id: NodeId) -> ContractResult<Z3Expr<'ctx>> {
            let node = self.graph.get_node(node_id).ok_or_else(|| {
                ContractError::InvalidExpression(format!("Node {} not found in graph", node_id))
            })?;

            match node {
                Node::Literal(lit) => self.convert_literal(lit),
                Node::Variable { name } => self.convert_variable(name),
                Node::Application { function, args } => self.convert_application(*function, args),
                _ => Err(ContractError::InvalidExpression(format!(
                    "Cannot convert node type {:?} to Z3 expression",
                    node
                ))),
            }
        }
        ...
        fn convert_variable(&self, name: &str) -> ContractResult<Z3Expr<'ctx>> {
            self.variables.get(name).cloned().ok_or_else(|| {
                ContractError::InvalidExpression(format!("Variable {} not declared", name))
            })
        }
        ...
        fn convert_application(
            ...
            func_id: NodeId,
            args: &[NodeId],
        ) -> ContractResult<Z3Expr<'ctx>> {
            // Get the function node
            let func_node = self.graph.get_node(func_id).ok_or_else(|| {
                ContractError::InvalidExpression(format!("Function node {} not found", func_id))
            })?;
        ...
        {
            if args.len() != 2 {
                return Err(ContractError::InvalidExpression(format!(
            ...
            F: Fn(&Bool<'ctx>, &Bool<'ctx>) -> Bool<'ctx>,
        {
            if args.len() != 2 {
                return Err(ContractError::InvalidExpression(format!(
                    "Logical operator expects 2 arguments, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_not(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "Not operator expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_modulo(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 2 {
                return Err(ContractError::InvalidExpression(format!(
                    "Modulo operator expects 2 arguments, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_abs(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "Abs function expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_not_equal(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 2 {
                return Err(ContractError::InvalidExpression(format!(
                    "Not-equal operator expects 2 arguments, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_zero_pred(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "zero? predicate expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_positive_pred(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "positive? predicate expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_negative_pred(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "negative? predicate expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_even_pred(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "even? predicate expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_odd_pred(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "odd? predicate expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_old(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "old() function expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_length(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "length function expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_nth(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 2 {
                return Err(ContractError::InvalidExpression(format!(
                    "nth function expects 2 arguments, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_member(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 2 {
                return Err(ContractError::InvalidExpression(format!(
                    "member? predicate expects 2 arguments, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_null_pred(&self, args: &[NodeId]) -> ContractResult<Z3Expr<'ctx>> {
            if args.len() != 1 {
                return Err(ContractError::InvalidExpression(format!(
                    "null? predicate expects 1 argument, got {}",
                    args.len()
                )));
            }
        ...
        fn convert_quantifier(
            ...
            if args.len() != 2 {
                return Err(ContractError::InvalidExpression(format!(
                    "Quantifier expects 2 arguments (bindings and body), got {}",
                    args.len()
                )));
            }
            ...
            let bindings_node = self.graph.get_node(args[0]).ok_or_else(|| {
                ContractError::InvalidExpression("Bindings not found".to_string())
            })?;
            ...
            let bound_vars = match bindings_node {
                Node::List(bindings) => {
                    ...
                    for binding_id in bindings {
                        let binding = self.graph.get_node(*binding_id).ok_or_else(|| {
                            ContractError::InvalidExpression("Invalid binding".to_string())
                        })?;
            ...
            for (var_name, sort) in &bound_vars {
                let z3_var = match sort {
                    ...
                    _ => {
                        return Err(ContractError::InvalidExpression(format!(
                            "Unsupported sort for quantified variable: {:?}",
                            sort
                        )))
                    }
                };
