...
impl<'a> QuantifierParser<'a> {
    ...
    pub fn parse_quantifier(
        &self,
        node_id: NodeId,
    ) -> ContractResult<Option<QuantifiedExpression>> {
        let node = self.graph.get_node(node_id).ok_or_else(|| {
            ContractError::InvalidExpression(format!("Node {} not found", node_id))
        })?;
            ...
            Node::Application { function, args } => {
                ...
                if args.len() != 2 {
                    return Err(ContractError::InvalidExpression(format!(
                        "{} expects 2 arguments: bindings and body",
                        func_name
                    )));
                }
    ...
    fn get_function_name(&self, node_id: NodeId) -> ContractResult<String> {
        let node = self.graph.get_node(node_id).ok_or_else(|| {
            ContractError::InvalidExpression(format!("Function node {} not found", node_id))
        })?;
    ...
    fn parse_bindings(&self, node_id: NodeId) -> ContractResult<Vec<(String, QuantifierDomain)>> {
        let node = self.graph.get_node(node_id).ok_or_else(|| {
            ContractError::InvalidExpression(format!("Bindings node {} not found", node_id))
        })?;
    ...
    fn get_variable_name(&self, node_id: NodeId) -> ContractResult<String> {
        let node = self.graph.get_node(node_id).ok_or_else(|| {
            ContractError::InvalidExpression(format!("Variable node {} not found", node_id))
        })?;
    ...
    fn parse_domain(&self, node_id: NodeId) -> ContractResult<QuantifierDomain> {
        let node = self.graph.get_node(node_id).ok_or_else(|| {
            ContractError::InvalidExpression(format!("Domain node {} not found", node_id))
        })?;
    ...
    fn get_integer_literal(&self, node_id: NodeId) -> ContractResult<i64> {
        let node = self.graph.get_node(node_id).ok_or_else(|| {
            ContractError::InvalidExpression(format!("Integer node {} not found", node_id))
        })?;
...
impl QuantifierBuilder {
    ...
    fn build_quantifier(
        ...
        for (var_name, domain) in vars {
            ...
            let binding = self
                .graph
                .add_node(Node::List(vec![var_node, domain_node]))
                .expect("Failed to create binding node");
        ...
        let bindings = self
            .graph
            .add_node(Node::List(binding_nodes))
            .expect("Failed to create bindings list");
    ...
    fn create_domain_node(&mut self, domain: QuantifierDomain) -> NodeId {
        match domain {
            QuantifierDomain::Unspecified => self
                .graph
                .add_node(Node::Variable {
                    name: "Int".to_string(),
                })
                .expect("Failed to create Int type node"),
            QuantifierDomain::Boolean => self
                .graph
                .add_node(Node::Variable {
                    name: "Bool".to_string(),
                })
                .expect("Failed to create Bool type node"),
            QuantifierDomain::IntRange(min, max) => {
                ...
                let min_node = self
                    .graph
                    .add_node(Node::Literal(fluentai_core::ast::Literal::Integer(min)))
                    .expect("Failed to create min node");
                let max_node = self
                    .graph
                    .add_node(Node::Literal(fluentai_core::ast::Literal::Integer(max)))
    ...
    fn test_quantifier_builder() {
        ...
        let quantified = builder.forall(
            ...
            |builder, vars| {
                ...
                let zero = builder
                    .graph
                    .add_node(Node::Literal(fluentai_core::ast::Literal::Integer(0)))
