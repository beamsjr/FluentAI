...
#[cfg(feature = "static")]
use z3::{
    ast::{Ast, Bool as Z3Bool, Dynamic as Z3Expr, Int as Z3Int},
    Context, Solver,
};

use crate::errors::{ContractError, ContractResult};
use crate::simplify::simplify;
use crate::symbolic_execution::{PathConstraint, SymbolicState, SymbolicType, SymbolicValue};
...
impl<'ctx> IncrementalSolver<'ctx> {
    ...
    pub fn add_constraint(&mut self, constraint: &PathConstraint) -> ContractResult<()> {
        ...
        let bool_constraint = z3_constraint
            .as_bool()
    ...
    pub fn check(&self) -> ContractResult<bool> {
        match self.solver.check() {
            ...
            z3::SatResult::Unknown => Err(ContractError::Other("Z3 returned unknown".to_string())),
        }
    }
        ...
        match value {
            SymbolicValue::Concrete(lit) => {
                ...
                match lit {
                    ...
                    _ => Err(ContractError::Other(format!(
                        "Unsupported literal type in Z3: {:?}",
                        lit
                    ))),
                }
            }
            ...
            SymbolicValue::BinOp { op, left, right } => {
                ...
                match op.as_str() {
                    ...
                    "+" => {
                        let l = left_expr
                            .as_int()
                            .ok_or_else(|| ContractError::Other("Expected int".to_string()))?;
                        let r = right_expr
                            .as_int()
                            .ok_or_else(|| ContractError::Other("Expected int".to_string()))?;
                        Ok((l + r).into())
                    ...
                    "and" => {
                        let l = left_expr
                            .as_bool()
                            .ok_or_else(|| ContractError::Other("Expected bool".to_string()))?;
                        let r = right_expr
                            .as_bool()
                            .ok_or_else(|| ContractError::Other("Expected bool".to_string()))?;
                        Ok((l & r).into())
            ...
            SymbolicValue::UnaryOp { op, operand } => {
                ...
                match op.as_str() {
                    "not" => {
                        let bool_expr = operand_expr.as_bool().ok_or_else(|| {
                            ContractError::Other("Expected bool for not".to_string())
                        })?;
                        Ok(bool_expr.not().into())
                    }
                    "-" => {
                        let int_expr = operand_expr.as_int().ok_or_else(|| {
                            ContractError::Other("Expected int for negation".to_string())
                        })?;
                        Ok((-int_expr).into())
                    }
                    _ => Err(ContractError::Other(format!(
            ...
            _ => Err(ContractError::Other(format!(
                "Unsupported symbolic value for Z3: {:?}",
                value
            ))),
        }
    }
...
mod tests {
    use super::*;
    use crate::symbolic_execution::{PathConstraint, SymbolicState};
    use fluentai_core::ast::Literal;
