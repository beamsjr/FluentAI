...
fn test_xor_composition() {
    ...
    let contract_a = Contract {
        ...
        preconditions: vec![ContractCondition::new(
    ...
    assert!(xor_contract.preconditions[0]
        .message
        .as_ref()
        .unwrap()
...
fn test_xor_composition_requires_two_contracts() {
    ...
    let result = hierarchy.compose_contracts(&["single".to_string()], CompositionType::ExclusiveOr);

    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("exactly 2"));

    // XOR with three contracts should also fail
    hierarchy.add_contract(Contract::new(
        "second".to_string(),
        NodeId(NonZeroU32::new(1).unwrap()),
    ));
    hierarchy.add_contract(Contract::new(
...
fn test_implication_composition() {
    ...
    assert!(impl_contract.preconditions[0]
        .message
        .as_ref()
        .unwrap()
...
fn test_implication_empty_conditions() {
    ...
    let result = hierarchy.compose_contracts(
        &[
            "no_preconditions".to_string(),
            "with_preconditions".to_string(),
        ],
        CompositionType::Implication,
    );
...
fn test_complex_composition_chain() {
    ...
    for i in 1..=4 {
        let contract = Contract {
            ...
            preconditions: vec![ContractCondition::new(
                NodeId(NonZeroU32::new(i * 2).unwrap()),
                ContractKind::Precondition,
            )
            .with_message(format!("pre_{}", i))],
            postconditions: vec![ContractCondition::new(
                NodeId(NonZeroU32::new(i * 2 + 1).unwrap()),
                ContractKind::Postcondition,
            )
