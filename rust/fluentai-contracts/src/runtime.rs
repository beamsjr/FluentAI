...
use fluentai_core::{ast::Graph, value::Value};
...
impl RuntimeVerifier {
    ...
    pub fn register_contract(&mut self, contract: Contract) {
        debug!(
            "Registering contract for function: {}",
    ...
    pub fn register_contract_validated(&mut self, contract: Contract) -> ContractResult<()> {
        debug!(
            "Registering and validating contract for function: {}",
            contract.function_name
        );
    ...
    fn verify_condition(
        ...
        let graph = self.ast_graph.as_ref().ok_or_else(|| {
...
impl VerificationContext {
    ...
    pub fn post(function_name: String, arguments: Vec<Value>, return_value: Value) -> Self {
        Self {
            function_name,
            arguments,
