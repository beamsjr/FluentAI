...
fn test_let_pure() {
    ...
    let let_expr = builder.add_let(
        vec![("x".to_string(), val1), ("y".to_string(), val2)],
        body
    );
...
fn test_contract_node_pure() {
    ...
    let contract_node = graph.add_node(Node::Contract {
...
fn test_validate_contract_purity_impure_precondition() {
    ...
    assert!(result.unwrap_err().to_string().contains("Precondition contains impure expression"));
}
...
fn test_validate_contract_purity_impure_postcondition() {
    ...
    let result = checker.validate_contract_purity(&contract);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Postcondition contains impure expression"));
}
...
fn test_validate_contract_purity_impure_invariant() {
    ...
    let result = checker.validate_contract_purity(&contract);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Invariant contains impure expression"));
}
