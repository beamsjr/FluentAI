...
impl<'a> FrameConditionManager<'a> {
    ...
    pub fn extract_from_contract(&mut self, contract: &Contract) -> ContractResult<FrameCondition> {
        ...
        self.frame_conditions
            .insert(func_name.clone(), frame.clone());

        Ok(frame)
    }

    /// Analyze an expression to find modifications
    fn analyze_modifications(
        &self,
        node_id: NodeId,
        frame: &mut FrameCondition,
    ) -> ContractResult<()> {
                ...
                if let Some(Node::Variable { name }) = self.graph.get_node(*function) {
                    match name.as_str() {
                        ...
                        "vector-set!" | "array-set!" => {
                            // Array modification
                            if args.len() >= 2 {
                                if let Some(Node::Variable { name: arr_name }) =
                        ...
                        "set-field!" | "." if args.len() >= 2 => {
                            // Field modification
                            if let Some(Node::Variable { name: obj_name }) =
                                self.graph.get_node(args[0])
                            {
                                if let Some(Node::Variable { name: field_name }) =
    ...
    fn extract_index(&self, node_id: NodeId) -> IndexExpr {
        if let Some(node) = self.graph.get_node(node_id) {
            match node {
                Node::Literal(lit) => {
                    if let fluentai_core::ast::Literal::Integer(n) = lit {
