...
impl ExecutionTree {
    ...
    pub fn get_statistics(&self) -> TreeStatistics {
        ...
        let satisfiable_paths = self.nodes.values()
            .filter(|n| n.is_leaf && n.is_satisfiable == Some(true))
            .count();
        let unsatisfiable_paths = self.nodes.values()
        ...
        let avg_constraints = if leaf_nodes > 0 {
            self.nodes.values()
                ...
                .sum::<usize>() as f64 / leaf_nodes as f64
        } else {
            0.0
        };
...
impl TreeBuilder {
    ...
    fn add_state(&mut self, state: SymbolicState) -> ContractResult<()> {
        ...
        if let Some(last_constraint) = state.path_constraints.last() {
            let node_id = self.tree.add_node(parent_id, last_constraint.clone(), state.clone())?;
...
mod tests {
    ...
    fn test_tree_construction() {
        ...
        state1.add_constraint(
            SymbolicValue::BinOp {
                op: ">".to_string(),
                left: Box::new(SymbolicValue::Symbolic { name: "x".to_string(), ty: None }),
                right: Box::new(SymbolicValue::Concrete(Literal::Integer(0))),
            },
            true
        );
        
        let node1 = tree.add_node(0, state1.path_constraints[0].clone(), state1).unwrap();
