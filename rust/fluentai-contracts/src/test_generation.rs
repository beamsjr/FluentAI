                ...
                match (op.as_str(), left.as_ref(), right.as_ref()) {
                    (
                        ...
                        if expected {
                            ...
                            info.upper_bound =
                                Some(info.upper_bound.map_or(*n - 1, |b| b.min(*n - 1)));
                        }
                    }
                    (
                        ">",
                        ...
                        if expected {
                            let info = param_info.entry(name.clone()).or_default();
                            info.lower_bound =
                                Some(info.lower_bound.map_or(*n + 1, |b| b.max(*n + 1)));
                        }
                    }
...
pub fn format_test_cases(
    ...
    match language {
        TestLanguage::FluentAi => format_fluentai_tests(test_cases, function_name),
...
/// Format test cases as FluentAi code
fn format_fluentai_tests(test_cases: &[TestCase], function_name: &str) -> String {
...
/// Format a literal value for FluentAi
