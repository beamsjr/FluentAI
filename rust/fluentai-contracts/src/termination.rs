...
use crate::{contract::Contract, errors::ContractResult};
...
impl<'a> TerminationChecker<'a> {
    ...
    pub fn analyze_contract(&mut self, contract: &Contract) -> ContractResult<TerminationResult> {
        ...
        if !self.is_recursive(func_name) {
            ...
            self.termination_cache
                .insert(func_name.clone(), result.clone());
            return Ok(result);
        }
    ...
    fn analyze_recursive_function(
        ...
        Ok(TerminationResult::Unknown {
            reason: "Could not find termination measure or prove structural termination"
                .to_string(),
        })
    }

    /// Extract termination measure from contract if specified
    fn extract_termination_measure(
        &self,
        _contract: &Contract,
    ) -> ContractResult<Option<TerminationMeasure>> {
    ...
    fn infer_structural_termination(
        &self,
        _func_name: &str,
        body: NodeId,
    ) -> ContractResult<Option<TerminationResult>> {
