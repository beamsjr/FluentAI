...
impl<'a> ConditionEvaluator<'a> {
    ...
    pub fn evaluate(&self, node_id: NodeId) -> ContractResult<Value> {
        let node = self.graph.get_node(node_id).ok_or_else(|| {
            ContractError::VerificationError(format!("Invalid node ID in contract: {:?}", node_id))
        })?;
        ...
        match node {
            ...
            _ => Err(ContractError::VerificationError(format!(
                "Unsupported node type in contract condition: {:?}",
                node
            ))),
        }
    }
    ...
    fn evaluate_list(&self, elements: &[NodeId]) -> ContractResult<Value> {
        let values: Result<Vec<_>, _> = elements.iter().map(|&elem| self.evaluate(elem)).collect();
        ...
        match (&args[0], &args[1]) {
            (Value::List(list), Value::Integer(idx)) => {
                ...
                if idx < list.len() {
                    ...
                    Err(ContractError::VerificationError(format!(
