...
impl StateMachine {
    ...
    pub fn initial_states(&self) -> Vec<&State> {
        self.states.values().filter(|s| s.is_initial).collect()
    }
    ...
    pub fn find_deadlocks(&self) -> Vec<&State> {
        self.states
            .values()
            .filter(|state| !state.is_final && self.transitions_from(&state.id).is_empty())
            .collect()
    }
    ...
    pub fn to_temporal_formula(&self) -> TemporalFormula {
        ...
        for state in self.states.values() {
            for inv in &state.invariants {
                ...
                formulas.push(always(implies(atom(state_pred), atom(inv.clone()))));
            }
        }
    ...
    fn transition_formula(&self, trans: &Transition) -> TemporalFormula {
        ...
        implies(and(ante_preds), next(and(post_preds)))
    }
}
...
impl StateMachineVerifier {
    ...
    pub fn verify_contract(
        ...
        let contract = self.contracts.get(contract_name).ok_or_else(|| {
            ContractError::Other(format!("Contract '{}' not found", contract_name))
        })?;
