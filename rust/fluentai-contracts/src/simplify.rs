...
pub fn simplify(value: &SymbolicValue) -> SymbolicValue {
    match value {
        ...
        SymbolicValue::Unknown => value.clone(),
...
fn simplify_unaryop(op: &str, operand: SymbolicValue) -> SymbolicValue {
    ...
    match op {
        "not" => {
            // not (not x) = x
            if let SymbolicValue::UnaryOp { op: inner_op, operand: inner } = &operand {
                if inner_op == "not" {
                    return (**inner).clone();
                }
            }
    ...
    for part in parts {
        ...
        if let SymbolicValue::Concrete(Literal::String(s)) = &simplified {
            ...
            if !current_string.is_empty() {
                simplified_parts.push(SymbolicValue::Concrete(Literal::String(current_string.clone())));
...
mod tests {
    ...
    fn test_arithmetic_simplification() {
        // x + 0 = x
        let x = SymbolicValue::Symbolic { name: "x".to_string(), ty: None };
        ...
        assert_eq!(simplify(&expr), SymbolicValue::Concrete(Literal::Integer(5)));
    }
    ...
    fn test_conditional_simplification() {
        let x = SymbolicValue::Symbolic { name: "x".to_string(), ty: None };
        let y = SymbolicValue::Symbolic { name: "y".to_string(), ty: None };
        ...
        let cond = SymbolicValue::Conditional {
            condition: Box::new(SymbolicValue::Symbolic { name: "c".to_string(), ty: None }),
            then_val: Box::new(x.clone()),
