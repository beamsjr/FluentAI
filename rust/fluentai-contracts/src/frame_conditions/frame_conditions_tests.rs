...
fn test_frame_condition_builder_allocation() {
    let frame = FrameConditionBuilder::new().allows_allocation().build();
...
fn test_check_modification_allowed_no_frame() {
    ...
    let result = manager
        .check_modification_allowed("unknown_func", &Modification::Variable("x".to_string()));
    assert!(result.is_ok());
...
fn test_check_modification_allowed_variable() {
    ...
    let result =
        manager.check_modification_allowed("func", &Modification::Variable("x".to_string()));
    assert!(result.is_ok());
    assert!(result.unwrap());

    // Disallowed modification
    let result =
        manager.check_modification_allowed("func", &Modification::Variable("z".to_string()));
    assert!(result.is_ok());
    assert!(!result.unwrap());
}
...
fn test_check_modification_allowed_field() {
    ...
    assert!(manager
        .check_modification_allowed("func", &Modification::Field(allowed))
        .unwrap());
    assert!(!manager
        .check_modification_allowed("func", &Modification::Field(disallowed))
        .unwrap());
}
...
fn test_check_modification_allowed_allocation() {
    ...
    assert!(manager
        .check_modification_allowed("func", &Modification::Allocation)
        .unwrap());
    assert!(!manager
        .check_modification_allowed("func", &Modification::Deallocation)
        .unwrap());
}
...
fn test_extract_index_default() {
    let mut graph = create_test_graph();
    let node = graph
        .add_node(Node::Literal(Literal::Float(3.14)))
        .expect("Failed to add node");
