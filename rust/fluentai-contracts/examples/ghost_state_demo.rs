...
use fluentai_contracts::{
    Contract, ContractCondition, ContractKind, GhostStateBuilder, GhostStateManager, GhostVariable,
    HistoryVariable,
};
...
fn demo_history_variables() {
    ...
    let body = graph
        .add_node(Node::Variable {
            name: "update-temp-body".to_string(),
        })
        .expect("Failed to add node");
...
fn demo_bank_account() {
    ...
    let account = graph
        ...
        .expect("Failed to add node");
    let deposit_body = graph
        .add_node(Node::Variable {
            name: "deposit-body".to_string(),
        })
        .expect("Failed to add node");
    let plus = graph
        .add_node(Node::Variable {
    ...
    let new_balance = graph
        .add_node(Node::Application {
            function: plus,
            args: vec![old_balance, amount],
        })
        .expect("Failed to add node");
    let balance_postcond = graph
        .add_node(Node::Application {
            function: eq,
    ...
    let new_deposits = graph
        .add_node(Node::Application {
            function: plus,
            args: vec![old_deposits, amount],
        })
        .expect("Failed to add node");
    let deposits_postcond = graph
        .add_node(Node::Application {
            function: eq,
    ...
    deposit_contract.add_postcondition(ContractCondition {
    ...
    deposit_contract.add_invariant(ContractCondition {
        expression: invariant,
