...
use fluentai_contracts::{
    Contract, FieldAccess, FrameCondition, FrameConditionBuilder, FrameConditionManager,
    HeapRegion, IndexExpr,
};
...
fn demo_field_modification() {
    ...
    let body = graph
        .add_node(Node::Variable {
            name: "update-balance-body".to_string(),
        })
        .expect("Failed to add node");
    ...
    println!(
        "  Can modify account.balance? {}",
        frame.modifies_fields.contains(&allowed_field)
    );
    println!(
        "  Can modify account.id? {}",
        frame.modifies_fields.contains(&disallowed_field)
    );
}
