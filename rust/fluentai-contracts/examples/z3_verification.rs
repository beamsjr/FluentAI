...
use fluentai_contracts::{
    Contract, ContractCondition, ContractKind, StaticVerifier, VerificationResult,
};
...
fn simple_contract_demo() -> Result<()> {
    ...
    let gt_op = graph.add_node(Node::Variable {
        name: ">".to_string(),
    });
    ...
    let result_var = graph.add_node(Node::Variable {
        name: "result".to_string(),
    });
    let zero2 = graph.add_node(Node::Literal(Literal::Integer(0)));
    let gt_op2 = graph.add_node(Node::Variable {
        name: ">".to_string(),
    });
...
fn contradiction_demo() -> Result<()> {
    ...
    let lt_op = graph.add_node(Node::Variable {
        name: "<".to_string(),
    });
...
fn counterexample_demo() -> Result<()> {
    ...
    let plus_op = graph.add_node(Node::Variable {
        name: "+".to_string(),
    });
