...
fn demo_sorted_array() {
    ...
    let contract_expr = builder.forall(
        ...
        |builder, vars| {
            ...
            // length(arr)
            let length_op = builder
                .graph
                .add_node(Node::Variable {
                    name: "length".to_string(),
                })
                .expect("Failed to add node");
            let arr_length = builder
                .graph
                .add_node(Node::Application {
                    function: length_op,
                    args: vec![arr],
    ...
    contract.add_postcondition(
        ContractCondition::new(contract_expr, ContractKind::Postcondition)
