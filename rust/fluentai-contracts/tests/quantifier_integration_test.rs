...
fn test_exists_quantifier() {
    ...
    // Create variables
    let lst = builder.graph.add_node(Node::Variable { name: "lst".to_string() }).expect("Failed to add node");
        ...
        |outer_builder, outer_vars| {
            ...
            outer_builder.exists(
                ...
                move |inner_builder, inner_vars| {
                    let y = inner_vars["y"];
                    let gt_op = inner_builder.graph.add_node(Node::Variable { name: ">".to_string() }).expect("Failed to add node");
