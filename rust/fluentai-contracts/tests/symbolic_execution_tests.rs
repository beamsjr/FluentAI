...
use fluentai_contracts::{
    symbolic_execution::{
        SymbolicValue, SymbolicState, SymbolicType, ListOperation
    },
};
...
fn test_symbolic_state() {
    ...
    match (&sym1, &sym2) {
        (
            SymbolicValue::Symbolic { name: n1, .. },
            SymbolicValue::Symbolic { name: n2, .. }
        ) => {
            assert_ne!(n1, n2);
...
fn test_symbolic_conditional() {
    ...
    match &conditional {
        SymbolicValue::Conditional { condition, then_val, else_val } => {
            assert!(matches!(condition.as_ref(), SymbolicValue::Concrete(Literal::Boolean(true))));
            assert!(matches!(then_val.as_ref(), SymbolicValue::Concrete(Literal::String(_))));
            assert!(matches!(else_val.as_ref(), SymbolicValue::Concrete(Literal::String(_))));
        }
...
fn test_list_operations() {
    ...
    assert!(matches!(head, SymbolicValue::ListOp { op: ListOperation::Head, .. }));
    assert!(matches!(tail, SymbolicValue::ListOp { op: ListOperation::Tail, .. }));
    assert!(matches!(cons, SymbolicValue::ListOp { op: ListOperation::Cons, .. }));
    assert!(matches!(length, SymbolicValue::ListOp { op: ListOperation::Length, .. }));
}
...
fn test_symbolic_map() {
    ...
    match map {
        SymbolicValue::Map(entries) => {
            ...
            match &entries[0] {
                (SymbolicValue::Concrete(Literal::String(k)), SymbolicValue::Concrete(Literal::Integer(v))) => {
                    assert_eq!(k, "key1");
                    assert_eq!(*v, 100);
                }
...
fn test_symbolic_state_fork() {
    ...
    // Add some state
    state.bindings.insert("x".to_string(), SymbolicValue::Concrete(Literal::Integer(42)));
