...
use fluentai_contracts::{Contract, ContractCondition, ContractKind, PurityChecker};
...
fn test_if_expression_purity() {
    ...
    let print_fn = graph
        ...
        .expect("Failed to add node");
    let msg = graph
        .add_node(Node::Literal(Literal::String("yes".to_string())))
        .expect("Failed to add node");
    let impure_then = graph
        .add_node(Node::Application {
            function: print_fn,
            args: vec![msg],
        })
...
fn test_contract_purity_validation() {
    ...
    let print_fn = graph
        ...
        .expect("Failed to add node");
    let msg = graph
        .add_node(Node::Literal(Literal::String("done".to_string())))
        .expect("Failed to add node");
    let impure_condition = graph
        .add_node(Node::Application {
            function: print_fn,
            args: vec![msg],
        })
