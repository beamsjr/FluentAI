...
use fluentai_contracts::{
    ...
    frame_conditions::{FrameCondition, FieldAccess, IndexExpr, HeapRegion},
};
...
fn test_contract_with_multiple_conditions() {
    let mut contract = Contract::new("safe_divide".to_string(), NodeId(NonZeroU32::new(1).unwrap()));
...
fn test_contract_all_condition_types() {
    let mut contract = Contract::new("complete_func".to_string(), NodeId(NonZeroU32::new(1).unwrap()));
    ...
    assert_eq!(contract.conditions_of_kind(ContractKind::Invariant).len(), 1);
