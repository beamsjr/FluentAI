...
use std::sync::Arc;
use claudelang_vm::{Bytecode, bytecode::{Instruction, Value, BytecodeChunk, Opcode}};
use claudelang_vm::di::*;
use claudelang_di::prelude::*;
use claudelang_effects::provider::EffectHandlerProvider;
use claudelang_effects::{EffectType, EffectHandler, EffectResult};
use anyhow::Result;
...
fn custom_effects_demo() -> Result<()> {
    ...
    impl EffectHandler for LoggingHandler {
        ...
        fn handle_sync(&self, operation: &str, args: &[claudelang_core::value::Value]) -> EffectResult {
            println!("  [EFFECT] {} called with {:?}", operation, args);
            Ok(claudelang_core::value::Value::Nil)
        }
    }
    ...
    effect_provider.register_factory(
        EffectType::IO,
