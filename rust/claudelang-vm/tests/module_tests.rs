...
fn test_compile_module_declaration() -> Result<()> {
    ...
    let has_begin_module = bytecode.chunks[0].instructions.iter()
        .any(|i| matches!(i.opcode, claudelang_vm::bytecode::Opcode::BeginModule));
    let has_end_module = bytecode.chunks[0].instructions.iter()
        .any(|i| matches!(i.opcode, claudelang_vm::bytecode::Opcode::EndModule));
...
fn test_compile_import_statement() -> Result<()> {
    ...
    let has_load_module = bytecode.chunks[0].instructions.iter()
        .any(|i| matches!(i.opcode, claudelang_vm::bytecode::Opcode::LoadModule));
    ...
    let import_count = bytecode.chunks[0].instructions.iter()
        .filter(|i| matches!(i.opcode, claudelang_vm::bytecode::Opcode::ImportBinding))
        .count();
...
fn test_compile_export_statement() -> Result<()> {
    ...
    let export_count = bytecode.chunks[0].instructions.iter()
        .filter(|i| matches!(i.opcode, claudelang_vm::bytecode::Opcode::ExportBinding))
        .count();
...
fn test_compile_qualified_variable() -> Result<()> {
    ...
    let has_load_qualified = bytecode.chunks[0].instructions.iter()
        .any(|i| matches!(i.opcode, claudelang_vm::bytecode::Opcode::LoadQualified));
...
fn test_vm_module_value_type() -> Result<()> {
    use claudelang_vm::bytecode::{Bytecode, BytecodeChunk, Instruction, Opcode, Value};
...
fn test_vm_export_binding() -> Result<()> {
    ...
    match result {
        claudelang_vm::bytecode::Value::Int(n) => assert_eq!(n, 42),
...
fn test_module_isolation() -> Result<()> {
    ...
    match result {
        claudelang_vm::bytecode::Value::Int(n) => assert_eq!(n, 10),
