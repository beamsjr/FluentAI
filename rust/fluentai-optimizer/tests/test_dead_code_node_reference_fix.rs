...
fn test_dead_code_elimination_invalid_node_reference() {
    ...
    let unused_var2 = graph
        .add_node(Node::Variable {
            name: "unused2".to_string(),
        })
        .expect("Failed to add node");
    let unused_var3 = graph
        .add_node(Node::Variable {
            name: "unused3".to_string(),
        })
        .expect("Failed to add node");

    // Create an application node that won't be reachable
    let unreachable_app = graph
    ...
    match optimizer.optimize(&graph) {
        Ok(optimized) => {
            ...
            let has_empty_bindings = optimized
                .nodes
                .values()
                .any(|node| matches!(node, Node::Let { bindings, .. } if bindings.is_empty()));
            assert!(
                has_empty_bindings,
                "Let node should have empty bindings after optimization"
            );
        ...
        Err(e) => {
            panic!(
                "Optimization should succeed now with our fix, but failed: {}",
                e
            );
        }
    }
        ...
        Node::Let { bindings, body } => {
            for (_, value_id) in bindings {
                assert!(
                    ...
                    "Binding value node {:?} doesn't exist",
                    value_id
                );
            }
            assert!(
                graph.get_node(*body).is_some(),
                "Let body node {:?} doesn't exist",
                body
            );
        }
        Node::Lambda { body, .. } => {
            assert!(
                graph.get_node(*body).is_some(),
                "Lambda body node {:?} doesn't exist",
                body
            );
        }
        Node::If {
            ...
            assert!(
                ...
                "Condition node {:?} doesn't exist",
                condition
            );
            assert!(
                graph.get_node(*then_branch).is_some(),
                "Then branch node {:?} doesn't exist",
                then_branch
            );
            assert!(
