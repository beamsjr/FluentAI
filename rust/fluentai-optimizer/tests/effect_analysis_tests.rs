...
fn test_multiple_effect_types() {
    ...
    let list_node = graph
        .add_node(Node::List(vec![io_effect, state_effect]))
        .expect("Failed to add node");
...
fn test_async_effect_detection() {
    ...
    let async_node = graph
        .add_node(Node::Async { body: body_node })
        .expect("Failed to add node");
...
fn test_concurrent_effect_detection() {
    ...
    let spawn_node = graph
        .add_node(Node::Spawn { expr: value })
        .expect("Failed to add node");
...
fn test_effect_free_program() {
    ...
    assert_eq!(
        non_pure_count, 0,
        "Found non-pure nodes in effect-free program"
    );
}
...
fn test_effect_analysis_completeness() {
    ...
    for node_id in ast.nodes.keys() {
        assert!(
            analysis.node_effects.contains_key(node_id),
            "Node {:?} missing from effect analysis",
            node_id
        );
    }
}
