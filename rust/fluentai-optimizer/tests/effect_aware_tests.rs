...
use fluentai_optimizer::passes::{effect_aware::EffectAwarePass, OptimizationPass};
...
fn test_hoist_pure_computations() {
    ...
    assert!(
        stats.contains("1 pure computations hoisted"),
        "Expected pure computations to be hoisted: {}",
        stats
    );
}
...
fn test_remove_duplicate_pure_expressions() {
    ...
    let stats = pass.stats();
    assert!(
        stats.contains("duplicates removed"),
...
fn test_preserve_effect_ordering() {
    ...
    let first = graph
        ...
        .expect("Failed to add node");
    let second = graph
        .add_node(Node::Literal(Literal::String("second".to_string())))
        .expect("Failed to add node");
    ...
    let let_node = graph
        .add_node(Node::Let {
            bindings: vec![("a".to_string(), a_value), ("b".to_string(), b_value)],
            body: result,
        })
        .expect("Failed to add node");
    ...
    assert_eq!(
        optimized.nodes.len(),
        before_count,
        "Effect ordering should be preserved"
    );

    // Verify no effects were reordered
    let stats = pass.stats();
    assert!(
        stats.contains("0 effects reordered"),
        "Expected no effects to be reordered: {}",
        stats
    );
}
...
fn test_mixed_pure_and_effectful() {
    ...
    let stats = pass.stats();
    assert!(
        stats.contains("pure computations hoisted"),
        "Expected pure computations to be hoisted: {}",
        stats
    );
}
