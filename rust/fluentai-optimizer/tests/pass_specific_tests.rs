...
fn test_constant_folding_arithmetic() {
    ...
    for (code, expected) in cases {
        ...
        println!("Testing {}: {} nodes, root={:?}", code, optimized.nodes.len(), optimized.root_id);
...
fn test_dead_code_elimination_unused_let() {
    ...
    println!("Dead code elimination: eliminated {} nodes", stats.dead_code_eliminated);
    println!("Optimized graph has {} nodes (from {})", optimized.nodes.len(), ast.nodes.len());
    assert!(stats.dead_code_eliminated >= 2, "Expected at least 2 nodes eliminated, got {}", stats.dead_code_eliminated);
}
...
fn test_cse_basic() {
    ...
    let add_count = optimized.nodes.values().filter(|node| {
        matches!(node, Node::Application { function, .. } if {
            optimized.get_node(*function)
...
fn test_tail_call_optimization_simple() {
    ...
    let stats = pipeline.stats();
    println!("Tail call optimization: {} tail calls optimized", stats.tail_calls_optimized);
    assert!(stats.tail_calls_optimized > 0, "Expected at least 1 tail call optimized, got {}", stats.tail_calls_optimized);
}
...
fn test_optimization_preserves_correctness() {
    ...
    for code in test_cases {
        ...
        assert_eq!(
            format!("{}", unopt_result), 
            format!("{}", opt_result),
            "Optimization changed result for: {}", code
        );
    }
}
