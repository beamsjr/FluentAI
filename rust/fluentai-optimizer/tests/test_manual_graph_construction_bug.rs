...
fn test_reachability_analysis_bug() {
    ...
    let let_node = graph
        .add_node(Node::Let {
            bindings: vec![("x".to_string(), x_var), ("y".to_string(), y_var)],
            body: lambda,
        })
        .expect("Failed to add node");
...
fn verify_node_references(graph: &Graph, node: &Node) {
    match node {
        ...
        Node::Letrec { bindings, body } => {
            ...
            assert!(
                graph.get_node(*body).is_some(),
                "Letrec body node {:?} doesn't exist",
                body
            );
        }
        Node::Lambda { body, .. } => {
        ...
        Node::Match { expr, branches } => {
            assert!(
                ...
                "Match expr node {:?} doesn't exist",
                expr
            );
            for (_, branch_body) in branches {
                assert!(
                    graph.get_node(*branch_body).is_some(),
                    "Match branch node {:?} doesn't exist",
                    branch_body
                );
            }
        }
