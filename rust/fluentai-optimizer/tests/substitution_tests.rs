    ...
    match node {
        Node::Application { function, args } => {
            assert!(graph.get_node(*function).is_some(), 
                    "Node {:?} has dangling function reference {:?}", node_id, function);
            for arg in args {
                assert!(graph.get_node(*arg).is_some(),
                        "Node {:?} has dangling arg reference {:?}", node_id, arg);
            }
        }
        Node::Lambda { body, .. } => {
            assert!(graph.get_node(*body).is_some(),
                    "Node {:?} has dangling body reference {:?}", node_id, body);
        }
        ...
        Node::If { condition, then_branch, else_branch } => {
            assert!(graph.get_node(*condition).is_some(),
                    "Node {:?} has dangling condition reference {:?}", node_id, condition);
            assert!(graph.get_node(*then_branch).is_some(),
                    "Node {:?} has dangling then reference {:?}", node_id, then_branch);
            assert!(graph.get_node(*else_branch).is_some(),
                    "Node {:?} has dangling else reference {:?}", node_id, else_branch);
        }
        Node::List(items) => {
            for item in items {
                assert!(graph.get_node(*item).is_some(),
                        "Node {:?} has dangling list item reference {:?}", node_id, item);
            }
        }
        Node::Match { expr, branches } => {
            assert!(graph.get_node(*expr).is_some(),
                    "Node {:?} has dangling expr reference {:?}", node_id, expr);
            for (_, branch) in branches {
                assert!(graph.get_node(*branch).is_some(),
                        "Node {:?} has dangling branch reference {:?}", node_id, branch);
            }
        }
        Node::Module { body, .. } => {
            assert!(graph.get_node(*body).is_some(),
                    "Node {:?} has dangling module body reference {:?}", node_id, body);
        }
        Node::Async { body } => {
            assert!(graph.get_node(*body).is_some(),
                    "Node {:?} has dangling async body reference {:?}", node_id, body);
        }
        Node::Await { expr } => {
            assert!(graph.get_node(*expr).is_some(),
                    "Node {:?} has dangling await expr reference {:?}", node_id, expr);
        }
        Node::Spawn { expr } => {
            assert!(graph.get_node(*expr).is_some(),
                    "Node {:?} has dangling spawn expr reference {:?}", node_id, expr);
        }
        Node::Send { channel, value } => {
            assert!(graph.get_node(*channel).is_some(),
                    "Node {:?} has dangling channel reference {:?}", node_id, channel);
            assert!(graph.get_node(*value).is_some(),
                    "Node {:?} has dangling value reference {:?}", node_id, value);
        }
        ...
        Node::Contract { preconditions, postconditions, invariants, .. } => {
            for pre in preconditions {
                assert!(graph.get_node(*pre).is_some(),
                        "Node {:?} has dangling precondition reference {:?}", node_id, pre);
            }
            for post in postconditions {
                assert!(graph.get_node(*post).is_some(),
                        "Node {:?} has dangling postcondition reference {:?}", node_id, post);
            }
            for inv in invariants {
                assert!(graph.get_node(*inv).is_some(),
                        "Node {:?} has dangling invariant reference {:?}", node_id, inv);
            }
        }
        Node::Effect { args, .. } => {
            for arg in args {
                assert!(graph.get_node(*arg).is_some(),
                        "Node {:?} has dangling effect arg reference {:?}", node_id, arg);
            }
        }
