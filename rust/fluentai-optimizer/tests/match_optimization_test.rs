...
use fluentai_optimizer::pipeline::{OptimizationPipeline, OptimizationConfig, OptimizationLevel};
use anyhow::Result;
    ...
    match node {
        Node::Match { expr, branches } => {
            ...
            if graph.get_node(*expr).is_none() {
                return Err(anyhow::anyhow!("Match expr references invalid node: {:?}", expr));
            }
            ...
            for (_, branch_body) in branches {
                if graph.get_node(*branch_body).is_none() {
                    return Err(anyhow::anyhow!("Match branch references invalid node: {:?}", branch_body));
                }
        ...
        Node::List(items) => {
            for item in items {
                if graph.get_node(*item).is_none() {
                    return Err(anyhow::anyhow!("List item references invalid node: {:?}", item));
                }
