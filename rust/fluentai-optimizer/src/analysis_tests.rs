...
mod tests {
    use crate::analysis::{
        ControlFlowGraph, DataFlowAnalysis, EffectAnalysis, AliasAnalysis, 
        TypeAnalysis, calculate_node_size, is_recursive_function
    };
    ...
    fn test_cfg_single_node() {
        ...
        assert!(cfg.predecessors.get(&node_id).map_or(true, |p| p.is_empty()));
    ...
    fn test_dataflow_shadowing() {
        ...
        let x_defs = dataflow.definitions.values()
    ...
    fn test_effect_analysis_multiple() {
        let code = "(do (effect IO:print \"start\") (effect State:set x 1) (effect IO:print \"end\"))";
        let graph = parse(code).unwrap();
        let effects = EffectAnalysis::analyze(&graph);
        
        // Should have both IO and State effects
        let mut all_effects: std::collections::HashSet<EffectType> = std::collections::HashSet::new();
