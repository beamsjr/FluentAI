...
use fluentai_core::ast::{EffectType, Graph, Literal, Node, NodeId, Pattern};
        ...
        match graph.get_node(node_id) {
            Some(node) => match node {
                ...
                Node::Application { function, args } => {
                    self.visit_application(graph, node_id, *function, args)
                }
                ...
                Node::Letrec { bindings, body } => {
                    self.visit_letrec(graph, node_id, bindings, *body)
                }
                Node::If {
                    condition,
                    then_branch,
                ...
                Node::Effect {
                    ...
                    operation,
                    args,
                } => self.visit_effect(graph, node_id, effect_type, operation, args),
...
mod tests {
    ...
    fn test_bounce_trampoline() {
        ...
        // Test nested calls
        let nested = Bounce::call(|| Bounce::call(|| Bounce::done(300)));
