...
pub struct GraphContainer {
    ...
    pub factories: HashMap<
        NodeId,
        Arc<dyn Fn(&GraphContainer) -> Box<dyn std::any::Any + Send + Sync> + Send + Sync>,
    >,
...
impl ServiceGraphBuilder {
    ...
    pub fn service(&mut self, interface: &str) -> ServiceNodeBuilder {
        let interface_id = self
            .create_symbol_node(interface)
            .expect("Failed to create interface node");
        ServiceNodeBuilder {
            builder: self,
            interface_id,
    ...
    fn create_symbol_node(&mut self, name: &str) -> Result<NodeId, DiError> {
        ...
        Ok(self
            ...
            .graph
            .add_node(node)
            .map_err(|_| DiError::Other("Graph node ID overflow".to_string()))?)
    }
...
impl<'a> ServiceNodeBuilder<'a> {
    ...
    pub fn implementation(mut self, impl_name: &str) -> Self {
        self.implementation_id = Some(
            self.builder
                .create_symbol_node(impl_name)
                .expect("Failed to create implementation node"),
        );
        self
    }
    ...
    pub fn depends_on(mut self, service: &str, kind: DependencyKind) -> Self {
        let target = self
            .builder
            .create_symbol_node(service)
            .expect("Failed to create dependency node");
