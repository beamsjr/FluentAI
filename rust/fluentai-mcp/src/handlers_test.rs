...
mod tests {
    ...
    async fn test_eval_error_division_by_zero() {
        ...
        assert!(err_msg.contains("error") || err_msg.contains("Error") || err_msg.contains("divide"));
    }
    ...
    async fn test_eval_missing_code() {
        ...
        let result = handle_eval(&mut state, Some(&args)).await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Missing code parameter"));
    }
    ...
    async fn test_search_docs_missing_query() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Missing query parameter"));
    }
