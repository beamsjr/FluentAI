...
use claudelang_effects::{
    EffectHandler, EffectHandlerProvider, EffectContext,
    provider::di::register_effect_services,
};
use std::sync::Arc;
...
fn main() -> anyhow::Result<()> {
    ...
    use claudelang_core::value::Value;
...
impl ApplicationService {
    ...
    fn run(&self) -> anyhow::Result<()> {
        ...
        // Get current time
        let time = self.effect_context.perform_sync(
            EffectType::Time,
            "now",
            &[]
        )?;
        ...
        let random = self.effect_context.perform_sync(
            EffectType::Random,
            "float",
            &[]
        )?;
...
impl EffectHandler for CustomIOHandler {
    fn effect_type(&self) -> claudelang_effects::EffectType {
        claudelang_effects::EffectType::IO
    }
    
    fn handle_sync(&self, operation: &str, args: &[claudelang_core::value::Value]) -> claudelang_effects::EffectResult {
        use claudelang_core::value::Value;
        ...
        match operation {
            ...
            _ => {
                ...
                let default = claudelang_effects::IOHandler::new();
...
impl EffectModule {
    ...
    fn register(&self, builder: &mut ContainerBuilder) -> anyhow::Result<()> {
        use claudelang_effects::EffectHandlerBuilder;
