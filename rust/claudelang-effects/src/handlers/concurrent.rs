            ...
            "send" => {
                if args.len() >= 2 {
                    if let Some(Value::String(channel_id)) = args.first() {
                        ...
                        Err(Error::Runtime("concurrent:send requires channel ID".to_string()))
                    }
                } else {
                    Err(Error::Runtime("concurrent:send requires 2 arguments".to_string()))
                }
            }
            "close" => {
                if let Some(Value::String(channel_id)) = args.first() {
        ...
        match operation {
            "receive" => {
                if let Some(Value::String(channel_id)) = args.first() {
                    ...
                    Err(Error::Runtime("concurrent:receive requires channel ID".to_string()))
                }
            }
            "select" => {
                ...
                if futures.is_empty() {
                    return Err(Error::Runtime("concurrent:select requires at least one channel".to_string()));
                }
            ...
            "spawn" => {
                ...
                // In a real implementation, we'd execute ClaudeLang code
            ...
            "join" => {
                if let Some(Value::String(goroutine_id)) = args.first() {
                    ...
                    Err(Error::Runtime("concurrent:join requires goroutine ID".to_string()))
                }
            }
