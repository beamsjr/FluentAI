//! ClaudeLang REPL binary

use claudelang_repl::{Repl, ReplConfig};
use clap::{Parser, ValueEnum};
use std::path::PathBuf;

#[derive(Parser)]
#[command(name = "claudelang-repl")]
#[command(about = "ClaudeLang Interactive REPL", long_about = None)]
struct Cli {
    /// Execution mode
...
fn main() -> anyhow::Result<()> {
    ...
    let mode = match cli.mode {
        ExecutionMode::Tree => claudelang_repl::ExecutionMode::TreeWalking,
        ExecutionMode::Bytecode => claudelang_repl::ExecutionMode::Bytecode,
        #[cfg(feature = "jit")]
        ExecutionMode::Jit => claudelang_repl::ExecutionMode::JIT,
    };
