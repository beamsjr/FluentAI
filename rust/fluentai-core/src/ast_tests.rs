...
mod tests {
    ...
    fn test_node_ids_iterator() {
        ...
        // Add multiple nodes
        let id1 = graph.add_node(Node::Literal(Literal::Integer(1)));
        let id2 = graph.add_node(Node::Literal(Literal::Integer(2)));
        let id3 = graph.add_node(Node::Literal(Literal::Integer(3)));
    ...
    fn test_nodes_iterator() {
        ...
        let int_id = graph.add_node(Node::Literal(Literal::Integer(42)));
        let str_id = graph.add_node(Node::Literal(Literal::String("hello".to_string())));
        let var_id = graph.add_node(Node::Variable { name: "x".to_string() });
        ...
        assert!(matches!(node_map.get(&int_id), Some(Node::Literal(Literal::Integer(42)))));
        assert!(matches!(node_map.get(&str_id), Some(Node::Literal(Literal::String(s))) if s == "hello"));
    ...
    fn test_dfs_traversal_all_node_types() {
        ...
        let let_body = graph.add_node(Node::Variable { name: "x".to_string() });
        ...
        let rec_value = graph.add_node(Node::Literal(Literal::Nil));
        let rec_body = graph.add_node(Node::Variable { name: "f".to_string() });
        ...
        // Test If traversal
        let cond = graph.add_node(Node::Literal(Literal::Boolean(true)));
        let then_br = graph.add_node(Node::Literal(Literal::Integer(1)));
        let else_br = graph.add_node(Node::Literal(Literal::Integer(2)));
        let if_node = graph.add_node(Node::If {
            condition: cond,
            then_branch: then_br,
            else_branch: else_br,
        ...
        let match_expr = graph.add_node(Node::Literal(Literal::Integer(5)));
        let branch1 = graph.add_node(Node::Literal(Literal::String("five".to_string())));
        let branch2 = graph.add_node(Node::Literal(Literal::String("other".to_string())));
        ...
        // Test List traversal
        let item1 = graph.add_node(Node::Literal(Literal::Integer(1)));
        let item2 = graph.add_node(Node::Literal(Literal::Integer(2)));
        let list = graph.add_node(Node::List(vec![item1, item2]));
        visited.clear();
        graph.dfs_from(list, |id, _| visited.push(id));
        assert_eq!(visited.len(), 3);
        
        // Test Effect traversal
        let arg1 = graph.add_node(Node::Literal(Literal::String("hello".to_string())));
        let arg2 = graph.add_node(Node::Literal(Literal::String("world".to_string())));
    ...
    fn test_dfs_with_cycles() {
        ...
        let a = graph.add_node(Node::Variable { name: "a".to_string() });
        let b = graph.add_node(Node::Variable { name: "b".to_string() });
        let c = graph.add_node(Node::Variable { name: "c".to_string() });
        
        // Create applications that reference each other
        let app1 = graph.add_node(Node::Application {
            function: a,
            args: vec![b],
        });
        let _app2 = graph.add_node(Node::Application {
            function: b,
            args: vec![c],
        });
        let app3 = graph.add_node(Node::Application {
            function: c,
    ...
    fn test_literal_equality() {
        ...
        assert_ne!(Literal::String("hello".to_string()), Literal::String("world".to_string()));
    ...
    fn test_lambda_node() {
        let mut graph = Graph::new();
        let body = graph.add_node(Node::Variable { name: "x".to_string() });
        let lambda = graph.add_node(Node::Lambda {
            params: vec!["x".to_string()],
            body,
        });
    ...
    fn test_list_node() {
        ...
        let item1 = graph.add_node(Node::Literal(Literal::Integer(1)));
        let item2 = graph.add_node(Node::Literal(Literal::Integer(2)));
        let item3 = graph.add_node(Node::Literal(Literal::Integer(3)));
        
        let list = graph.add_node(Node::List(vec![item1, item2, item3]));
    ...
    fn test_match_node() {
        let mut graph = Graph::new();
        let expr = graph.add_node(Node::Literal(Literal::Integer(42)));
        let branch1 = graph.add_node(Node::Literal(Literal::String("forty-two".to_string())));
        let branch2 = graph.add_node(Node::Literal(Literal::String("other".to_string())));
    ...
    fn test_effect_node() {
        let mut graph = Graph::new();
        let arg = graph.add_node(Node::Literal(Literal::String("Hello, World!".to_string())));
        ...
        if let Some(Node::Effect { effect_type, operation, args }) = graph.get_node(effect) {
    ...
    fn test_module_node() {
        ...
        if let Some(Node::Module { name, exports, body: b }) = graph.get_node(module) {
    ...
    fn test_async_constructs() {
        ...
        // Test Async node
        let body = graph.add_node(Node::Literal(Literal::Integer(42)));
        let async_node = graph.add_node(Node::Async { body });
        ...
        // Test Send node
        let value = graph.add_node(Node::Literal(Literal::String("message".to_string())));
        let send = graph.add_node(Node::Send { channel, value });
        
        if let Some(Node::Send { channel: c, value: v }) = graph.get_node(send) {
            assert_eq!(*c, channel);
            assert_eq!(*v, value);
        } else {
            panic!("Wrong node type");
    ...
    fn test_context_memory() {
        ...
        assert_eq!(graph.get_context_memory(node).unwrap().usage_stats.execution_count, 1);
    }
    ...
    fn test_import_export_nodes() {
        ...
        // Test Import node
        let import = graph.add_node(Node::Import {
            module_path: "std/math".to_string(),
            import_list: vec![
                ImportItem { name: "sin".to_string(), alias: None },
                ImportItem { name: "cos".to_string(), alias: Some("cosine".to_string()) },
            ],
            import_all: false,
        });
        
        if let Some(Node::Import { module_path, import_list, import_all }) = graph.get_node(import) {
        ...
        // Test Export node
        let export = graph.add_node(Node::Export {
            export_list: vec![
                ExportItem { name: "foo".to_string(), alias: None },
                ExportItem { name: "bar".to_string(), alias: Some("baz".to_string()) },
            ],
        });
    ...
    fn test_qualified_variable() {
        ...
        let qual_var = graph.add_node(Node::QualifiedVariable {
            module_name: "math".to_string(),
            variable_name: "pi".to_string(),
        });
        
        if let Some(Node::QualifiedVariable { module_name, variable_name }) = graph.get_node(qual_var) {
    ...
    fn test_await_node() {
        ...
        let body = graph.add_node(Node::Literal(Literal::Integer(42)));
        let async_expr = graph.add_node(Node::Async { body });
        
        let await_node = graph.add_node(Node::Await { expr: async_expr });
    ...
    fn test_spawn_node() {
        ...
        let computation = graph.add_node(Node::Literal(Literal::String("compute".to_string())));
        let spawn = graph.add_node(Node::Spawn { expr: computation });
    ...
    fn test_receive_node() {
        ...
        let channel = graph.add_node(Node::Channel);
        let receive = graph.add_node(Node::Receive { channel });
    ...
    fn test_letrec_node() {
        ...
        let _n_var = graph.add_node(Node::Variable { name: "n".to_string() });
        let fact_var = graph.add_node(Node::Variable { name: "fact".to_string() });
        
        let fact_body = graph.add_node(Node::Literal(Literal::Nil)); // Simplified
    ...
    fn test_contract_node() {
        ...
        let precond = graph.add_node(Node::Literal(Literal::Boolean(true)));
        let postcond = graph.add_node(Node::Literal(Literal::Boolean(true)));
        let invariant = graph.add_node(Node::Literal(Literal::Boolean(true)));
    ...
    fn test_nested_patterns() {
        ...
        match &outer_cons {
            Pattern::Constructor { name, patterns } => {
                ...
                match &patterns[1] {
                    Pattern::Constructor { name: inner_name, patterns: inner_patterns } => {
    ...
    fn test_graph_node_removal() {
        ...
        let node1 = graph.add_node(Node::Literal(Literal::Integer(1)));
        let node2 = graph.add_node(Node::Literal(Literal::Integer(2)));
        let node3 = graph.add_node(Node::Literal(Literal::Integer(3)));
    ...
    fn test_children_of_complex_nodes() {
        ...
        let expr = graph.add_node(Node::Literal(Literal::Integer(5)));
        let branch1 = graph.add_node(Node::Literal(Literal::String("five".to_string())));
        let branch2 = graph.add_node(Node::Literal(Literal::String("other".to_string())));
    ...
    fn test_variable_node_docs() {
        let var_node = Node::Variable { name: "x".to_string() };
    ...
    fn test_all_performance_hint_types() {
        let hints = vec![
            PerformanceHintType::ShouldInline,
            PerformanceHintType::ShouldUnroll,
            PerformanceHintType::CanVectorize,
            PerformanceHintType::CanParallelize,
            PerformanceHintType::ShouldMemoize,
            PerformanceHintType::MemoryAccessPattern("Sequential".to_string()),
    ...
    fn test_literal_documented_node() {
        use crate::documentation::{DocumentedNode, DocumentationCategory};
    ...
    fn test_default_graph() {
        ...
        let id = graph.add_node(Node::Literal(Literal::Nil));
        assert_eq!(id.get(), 1);
    }
    ...
    fn test_nodes_iterator_with_types() {
        ...
        // Add some nodes
        let id1 = graph.add_node(Node::Literal(Literal::Integer(1)));
        let id2 = graph.add_node(Node::Variable { name: "x".to_string() });
        let id3 = graph.add_node(Node::Literal(Literal::String("hello".to_string())));
        ...
        assert!(matches!(found.get(&id1), Some(Node::Literal(Literal::Integer(1)))));
    ...
    fn test_dfs_if_traversal() {
        ...
        let then_branch = graph.add_node(Node::Literal(Literal::String("positive".to_string())));
        let else_branch = graph.add_node(Node::Literal(Literal::String("non-positive".to_string())));
    ...
    fn test_dfs_match_traversal() {
        ...
        let x = graph.add_node(Node::Variable { name: "x".to_string() });
        let zero_branch = graph.add_node(Node::Literal(Literal::String("zero".to_string())));
        let one_branch = graph.add_node(Node::Literal(Literal::String("one".to_string())));
        let other_branch = graph.add_node(Node::Literal(Literal::String("other".to_string())));
    ...
    fn test_dfs_list_traversal() {
        ...
        let inner_list = graph.add_node(Node::List(vec![three, four]));
        let outer_list = graph.add_node(Node::List(vec![one, two, inner_list]));
    ...
    fn test_dfs_effect_traversal() {
        ...
        let world = graph.add_node(Node::Literal(Literal::String("World".to_string())));
    ...
    fn test_dfs_no_cycles() {
        ...
        let d = graph.add_node(Node::Literal(Literal::Integer(4)));
        let b = graph.add_node(Node::List(vec![d]));
        let c = graph.add_node(Node::List(vec![d]));
        let a = graph.add_node(Node::List(vec![b, c]));
    ...
    fn test_dfs_empty_collections() {
        ...
        let empty_let = graph.add_node(Node::Let {
            bindings: vec![],
            body: nil_body,
        });
        ...
        // Effect with no args
        let no_arg_effect = graph.add_node(Node::Effect {
