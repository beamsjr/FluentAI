...
use fluentai_core::ast::{Graph, Node, Literal, Pattern, RangePattern};
use anyhow::Result;

#[test]
fn test_guard_pattern_creation() {
    let mut graph = Graph::new();
    let condition = graph.add_node(Node::Literal(Literal::Boolean(true))).unwrap();
    
    let guard_pattern = Pattern::guard(Pattern::var("x"), condition);
    
    match guard_pattern {
        Pattern::Guard { pattern, condition: cond } => {
...
fn test_as_pattern_creation() {
    let as_pattern = Pattern::as_pattern("list", Pattern::cons(Pattern::var("h"), Pattern::var("t")));
...
fn test_range_pattern_creation() {
    ...
    match inclusive_range {
        Pattern::Range(RangePattern { start, end, inclusive }) => {
...
fn test_view_pattern_creation() {
    let mut graph = Graph::new();
    let abs_fn = graph.add_node(Node::Variable { name: "abs".to_string() }).unwrap();
    ...
    match guard_pattern {
        Pattern::Guard { pattern, condition: _ } => {
            match pattern.as_ref() {
                Pattern::As { binding, pattern: inner } => {
...
fn test_match_builder_with_guard() -> Result<()> {
    ...
    let value = graph.add_node(Node::Variable { name: "x".to_string() })?;
    let gt_fn = graph.add_node(Node::Variable { name: ">".to_string() })?;
    ...
    let non_positive = graph.add_node(Node::Literal(Literal::String("non-positive".to_string())))?;
...
fn test_match_builder_with_or_pattern() -> Result<()> {
    ...
    let value = graph.add_node(Node::Variable { name: "day".to_string() })?;
...
fn test_match_builder_with_range() -> Result<()> {
    ...
    let score = graph.add_node(Node::Variable { name: "score".to_string() })?;
...
fn test_match_builder_with_as_pattern() -> Result<()> {
    ...
    let whole_list = graph.add_node(Node::Variable { name: "whole".to_string() })?;
...
fn test_complex_pattern_combination() -> Result<()> {
    ...
    let input = graph.add_node(Node::Variable { name: "input".to_string() })?;
    ...
    let even_fn = graph.add_node(Node::Variable { name: "even?".to_string() })?;
    let is_even = graph.add_node(Node::Application {
        function: even_fn,
        args: vec![input],
    })?;
    
    let odd_fn = graph.add_node(Node::Variable { name: "odd?".to_string() })?;
    ...
    let match_node = graph.build_match()
        ...
        .branch(
            Pattern::guard(Pattern::int_range(1, 10), is_odd),
            small_odd,
        )
        .branch(
            Pattern::int_range(11, 50),
            medium,
        )
        .branch(
            Pattern::or(vec![
