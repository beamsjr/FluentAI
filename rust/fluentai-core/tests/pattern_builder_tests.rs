...
fn test_pattern_builder_methods() {
    ...
    assert!(matches!(bool_pattern, Pattern::Literal(Literal::Boolean(true))));
...
fn test_cons_and_nil_patterns() {
    ...
    assert!(matches!(nil_pattern, Pattern::Constructor { ref name, ref patterns } if name == "nil" && patterns.is_empty()));
...
fn test_match_builder_simple() -> Result<()> {
    ...
    if let Some(Node::Match { expr, branches }) = graph.get_node(match_node) {
        ...
        assert!(matches!(branches[0].0, Pattern::Literal(Literal::Integer(42))));
...
fn test_match_builder_multiple_cases() -> Result<()> {
    ...
    let value = graph.add_node(Node::Variable { name: "input".to_string() })?;
...
fn test_match_builder_with_patterns() -> Result<()> {
    ...
    let match_node = graph.build_match()
        ...
        .branch(Pattern::cons(Pattern::var("h"), Pattern::var("t")), cons_result)
        .build()?;
...
fn test_match_builder_error_no_expr() {
    ...
    let match_result = graph.build_match()
        .int_case(1, result)
        .build();
    
    assert!(match_result.is_err());
    assert!(match_result.unwrap_err().to_string().contains("Match expression required"));
}
...
fn test_match_builder_error_no_branches() -> Result<()> {
    ...
    assert!(match_result.is_err());
    assert!(match_result.unwrap_err().to_string().contains("at least one branch"));
    Ok(())
}
...
fn test_complex_pattern_matching_example() -> Result<()> {
    ...
    let empty_result = graph.add_node(Node::Literal(Literal::String("empty".to_string())))?;
    let has_nested = graph.add_node(Node::Literal(Literal::String("has nested list".to_string())))?;
...
fn test_old_vs_new_api_comparison() -> Result<()> {
    ...
    let match_node_new = graph2.build_match()
        .expr(value2)
