...
/// Compile a FluentAi graph to Web Components
pub fn compile_graph(
    graph: &Graph,
    imports: &mut HashSet<String>,
    helpers: &mut HashSet<String>,
) -> Result<String> {
    let mut compiler = WebComponentCompiler::new(graph, imports, helpers);
    
    if let Some(root_id) = graph.root_id {
        compiler.compile_node(root_id)
    } else {
        Err(CompilerError::CodegenError("No root node in graph".to_string()))
    }
}

struct WebComponentCompiler<'a> {
    graph: &'a Graph,
...
impl<'a> WebComponentCompiler<'a> {
    ...
    fn compile_node_impl(&mut self, node: &Node, _node_id: NodeId) -> Result<String> {
        match node {
            ...
            _ => Err(CompilerError::UnsupportedFeature(format!(
                "Node type {:?} not yet supported for Web Component compilation",
                node
            ))),
        }
    }
    ...
    fn compile_application(&mut self, function: NodeId, args: &[NodeId]) -> Result<String> {
        ...
        if let Some(Node::Variable { name }) = self.graph.get_node(function) {
            match name.as_str() {
                ...
                "ui:component" => return self.compile_component_definition(args),
                _ => {}
            }
        }
    ...
    fn compile_if(&mut self, condition: NodeId, then_branch: NodeId, else_branch: NodeId) -> Result<String> {
    ...
    fn compile_effect(&mut self, effect_type: &str, operation: &str, args: &[NodeId]) -> Result<String> {
        match effect_type {
            "Dom" => self.compile_dom_effect(operation, args),
            "State" => self.compile_state_effect(operation, args),
            _ => Err(CompilerError::UnsupportedFeature(format!(
                "Effect type {} not supported in Web Component compilation",
                effect_type
            ))),
        }
    }
    
    // Web Component-specific UI compilation methods
    fn compile_ui_element(&mut self, args: &[NodeId]) -> Result<String> {
        if args.len() < 1 {
            return Err(CompilerError::CodegenError("ui:element requires tag name".to_string()));
        }
        ...
        // Use native DOM APIs for Web Components
        let mut result = format!("(() => {{\n");
        result.push_str(&format!("  const el = document.createElement({});\n", tag));
        
        // Apply properties
        if props != "{}" {
            result.push_str(&format!("  Object.assign(el, {});\n", props));
        }
        
        // Add children
        if !children.is_empty() {
            result.push_str(&format!("  el.append({});\n", children));
        }
        
        result.push_str("  return el;\n})()");
        Ok(result)
    }
    
    fn compile_ui_text(&mut self, args: &[NodeId]) -> Result<String> {
        if args.is_empty() {
            return Ok("document.createTextNode(\"\")".to_string());
        }
    ...
    fn compile_ui_bind(&mut self, args: &[NodeId]) -> Result<String> {
        if args.len() < 2 {
            return Err(CompilerError::CodegenError("ui:bind requires property and value".to_string()));
        }
        
        let prop = self.compile_node(args[0])?;
        let value = self.compile_node(args[1])?;
        
        // Return object for property binding
        Ok(format!("{{ {}: {} }}", prop, value))
    }
    
    fn compile_ui_event(&mut self, args: &[NodeId]) -> Result<String> {
        if args.len() < 2 {
            return Err(CompilerError::CodegenError("ui:on requires event and handler".to_string()));
        }
        ...
        Ok(format!("{{ addEventListener: [{}, {}] }}", event, handler))
    }
    
    fn compile_ui_conditional(&mut self, args: &[NodeId]) -> Result<String> {
        if args.len() < 2 {
            return Err(CompilerError::CodegenError("ui:if requires condition and content".to_string()));
        }
    ...
    fn compile_ui_loop(&mut self, args: &[NodeId]) -> Result<String> {
        if args.len() < 3 {
            return Err(CompilerError::CodegenError("ui:for requires items, binding, and template".to_string()));
        }
        ...
        let binding = match self.graph.get_node(args[1]) {
            ...
            _ => return Err(CompilerError::CodegenError("ui:for binding must be a variable".to_string())),
        };
    ...
    fn compile_component_definition(&mut self, args: &[NodeId]) -> Result<String> {
        if args.is_empty() {
            return Err(CompilerError::CodegenError("ui:component requires component name".to_string()));
        }
        
        let name = match self.graph.get_node(args[0]) {
            Some(Node::Literal(Literal::String(s))) => s.clone(),
            Some(Node::Variable { name }) => name.clone(),
            _ => format!("FluentAiComponent{}", self.component_counter),
        };
        ...
        let mut result = format!("class {} extends HTMLElement {{\n", name);
        result.push_str("  constructor() {\n");
        result.push_str("    super();\n");
        result.push_str("    this.attachShadow({ mode: 'open' });\n");
        result.push_str("  }\n\n");
        
        result.push_str("  connectedCallback() {\n");
        result.push_str(&format!("    const content = {};\n", template));
        result.push_str("    if (content) {\n");
        result.push_str("      this.shadowRoot.appendChild(content);\n");
        result.push_str("    }\n");
        result.push_str("  }\n");
        result.push_str("}\n\n");
        
        // Register the component
        let tag_name = name.to_lowercase().replace("_", "-");
        result.push_str(&format!("customElements.define('{}', {});", tag_name, name));
    ...
    fn compile_dom_effect(&mut self, operation: &str, args: &[NodeId]) -> Result<String> {
        match operation {
            ...
            "set-attribute" => {
                if args.len() < 3 {
                    return Err(CompilerError::CodegenError("set-attribute requires element, name, and value".to_string()));
                }
            ...
            "add-event-listener" => {
                if args.len() < 3 {
                    return Err(CompilerError::CodegenError("add-event-listener requires element, event, and handler".to_string()));
                }
                let elem = self.compile_node(args[0])?;
                let event = self.compile_node(args[1])?;
                let handler = self.compile_node(args[2])?;
                Ok(format!("{}.addEventListener({}, {})", elem, event, handler))
            }
            _ => Err(CompilerError::UnsupportedFeature(format!("DOM operation {} not supported", operation))),
        }
    }
    ...
    fn compile_state_effect(&mut self, operation: &str, args: &[NodeId]) -> Result<String> {
        match operation {
            "reactive:ref" => {
                self.helpers.insert("createReactive".to_string());
                let initial = if args.is_empty() {
                    "null".to_string()
                } else {
                    self.compile_node(args[0])?
                };
                Ok(format!("createReactive({})", initial))
            }
            "reactive:get" => {
                if args.is_empty() {
                    return Err(CompilerError::CodegenError("reactive:get requires ref".to_string()));
                }
            ...
            "reactive:set" => {
                if args.len() < 2 {
                    return Err(CompilerError::CodegenError("reactive:set requires ref and value".to_string()));
                }
                let ref_str = self.compile_node(args[0])?;
                let value = self.compile_node(args[1])?;
                Ok(format!("{}.value = {}", ref_str, value))
            }
            _ => Err(CompilerError::UnsupportedFeature(format!("State operation {} not supported", operation))),
        }
    }
