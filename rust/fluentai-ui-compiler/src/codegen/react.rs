...
/// Compile a FluentAi graph to React
pub fn compile_graph(
    graph: &Graph,
    imports: &mut HashSet<String>,
    helpers: &mut HashSet<String>,
) -> Result<String> {
    let mut compiler = ReactCompiler::new(graph, imports, helpers);
...
impl<'a> ReactCompiler<'a> {
    ...
    fn compile_node_impl(&mut self, node: &Node, _node_id: NodeId) -> Result<String> {
        match node {
            ...
            _ => Err(CompilerError::UnsupportedFeature(format!(
                "Node type {:?} not yet supported for React compilation",
                node
            ))),
        }
    }
    ...
    fn compile_effect(&mut self, effect_type: &str, operation: &str, args: &[NodeId]) -> Result<String> {
        match effect_type {
            ...
            _ => Err(CompilerError::UnsupportedFeature(format!(
                "Effect type {} not supported in React compilation",
                effect_type
            ))),
        }
    }
    
    // React-specific UI compilation methods
    fn compile_ui_element(&mut self, args: &[NodeId]) -> Result<String> {
        ...
        if children.is_empty() {
            ...
            Ok(format!("React.createElement({}, {}, {})", tag, props, children))
        }
    }
    ...
    fn compile_ui_bind(&mut self, args: &[NodeId]) -> Result<String> {
        ...
        // In React, we typically use state hooks
    ...
    fn compile_ui_event(&mut self, args: &[NodeId]) -> Result<String> {
        ...
        // Convert to React event handler naming
        let event_name = event.trim_matches('"');
        let react_event = format!("on{}", 
            event_name.chars().next().unwrap().to_uppercase().collect::<String>() + 
            &event_name[1..]
        );
        
        Ok(format!("{{ {}: {} }}", react_event, handler))
    }
    ...
    fn compile_ui_loop(&mut self, args: &[NodeId]) -> Result<String> {
        ...
        // React requires keys for list items
        Ok(format!("{}.map(({}, index) => React.cloneElement({}, {{ key: index }}))", 
    ...
    fn compile_component(&mut self, args: &[NodeId]) -> Result<String> {
        ...
        if children.is_empty() {
            Ok(format!("React.createElement({}, {})", component, props))
        } else {
            Ok(format!("React.createElement({}, {}, {})", component, props, children))
        }
    }
    ...
    fn compile_state_effect(&mut self, operation: &str, args: &[NodeId]) -> Result<String> {
        match operation {
            "reactive:ref" => {
                self.imports.insert("react".to_string());
                let initial = if args.is_empty() {
                    "null".to_string()
                } else {
                    self.compile_node(args[0])?
                };
                Ok(format!("React.useState({})[0]", initial))
            }
            "reactive:get" => {
                if args.is_empty() {
            ...
            "reactive:set" => {
                ...
                let setter = self.compile_node(args[0])?;
                let value = self.compile_node(args[1])?;
                Ok(format!("{}Set({})", setter.trim_end_matches("[0]"), value))
            }
