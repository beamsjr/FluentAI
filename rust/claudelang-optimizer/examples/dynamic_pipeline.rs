...
fn basic_pipeline_demo() -> Result<()> {
    ...
    println!("Optimization time: {} µs", pipeline.stats().optimization_time_us);
    println!();
...
fn custom_pass_demo() -> Result<()> {
    ...
    impl claudelang_optimizer::passes::OptimizationPass for CustomConstantPass {
        fn name(&self) -> &str {
...
fn runtime_composition_demo() -> Result<()> {
    ...
    impl claudelang_optimizer::passes::OptimizationPass for MonitoringPass {
        fn name(&self) -> &str {
            &self.pass_name
        }
        ...
        fn run(&mut self, graph: &Graph) -> Result<Graph> {
            ...
            println!("  [{}] Processing {} nodes", self.pass_name, self.nodes_seen);
            Ok(graph.clone())
        }
