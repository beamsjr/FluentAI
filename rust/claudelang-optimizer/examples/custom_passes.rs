...
use claudelang_optimizer::*;
use claudelang_optimizer::di::*;
...
impl OptimizationPass for StringInterningPass {
    ...
    fn run(&mut self, graph: &Graph) -> Result<Graph> {
        ...
        println!("  String interning: found {} duplicate strings", self.replacements);
        Ok(new_graph)
    }
    
    fn is_applicable(&self, graph: &Graph) -> bool {
        // Only run if there are string literal nodes
        graph.nodes.values().any(|n| matches!(n, Node::Literal(Literal::String(_))))
    }
}
...
impl OptimizationPass for AlgebraicSimplificationPass {
    ...
    fn run(&mut self, graph: &Graph) -> Result<Graph> {
        ...
        println!("  Algebraic simplification: {} simplifications", self.simplifications);
        Ok(new_graph)
    }
    
    fn is_applicable(&self, graph: &Graph) -> bool {
        // Check if there are any integer literals or applications
        graph.nodes.values().any(|n| matches!(n, Node::Literal(Literal::Integer(_)) | Node::Application { .. }))
    }
}
    ...
    fn run(&mut self, graph: &Graph) -> Result<Graph> {
        ...
        for (id, node) in &new_graph.nodes {
            if let Node::Application { function, args } = node {
                ...
                let const_args: Vec<bool> = args.iter()
                    .map(|arg_id| {
                        matches!(
                            new_graph.nodes.get(arg_id),
                            Some(Node::Literal(_))
                        )
                    })
                    .collect();
        ...
        println!("  Function specialization: {} potential specializations", self.specializations);
        Ok(new_graph)
    }
    
    fn is_applicable(&self, graph: &Graph) -> bool {
        graph.nodes.values().any(|n| matches!(n, Node::Application { .. }))
    }
}
...
fn combined_pipeline_demo() -> Result<()> {
    ...
    let mut pipeline = OptimizationPipelineBuilder::with_level(OptimizationLevel::Standard)
        ...
        .register_pass("algebraic_simplification", || Box::new(AlgebraicSimplificationPass::new()))
        .register_pass("function_specialization", || Box::new(FunctionSpecializationPass::new(10)))
        .with_default_passes()
        .build();
