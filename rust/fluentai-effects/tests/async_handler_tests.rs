...
async fn test_async_handler_await_unknown_promise() {
    ...
    let result = handler.handle_async("await", &[
        Value::String("unknown-promise-id".to_string()),
    ]).await;
...
async fn test_async_handler_all_operation() {
    ...
    let result = handler.handle_async("all", &[
        promise1,
        promise2,
        promise3,
    ]).await;
...
async fn test_async_handler_all_with_non_promise() {
    ...
    let result2 = handler.handle_async("all", &[
        Value::Integer(123),
        Value::Float(456.0),
        Value::List(vec![]),
    ]).await;
...
async fn test_async_handler_race_operation() {
    ...
    let result = handler.handle_async("race", &[
        promise1,
        promise2,
    ]).await;
...
async fn test_async_handler_promise_not_yet_resolved() {
    ...
    let promise_id = handler.handle_sync("promise", &[
...
async fn test_async_handler_mixed_operations() {
    ...
    let resolve_result = handler.handle_sync("resolve", &[
        promise.clone(),
