...
fn test_effect_handler_provider() {
    ...
    // Test IO effect
    let result = context.perform_sync(EffectType::IO, "print", &[Value::String("Hello".to_string())]);
    assert!(result.is_ok());
    
    // Test State effect
    let result = context.perform_sync(EffectType::State, "set", &[
        Value::String("key".to_string()),
        Value::Integer(42)
    ]);
    assert!(result.is_ok());
    
    let result = context.perform_sync(EffectType::State, "get", &[Value::String("key".to_string())]);
    assert!(result.is_ok());
...
fn test_hierarchical_providers() {
    ...
    let parent = Arc::new(EffectHandlerBuilder::new()
...
fn test_effect_context_with_multiple_handlers() {
    ...
    let io_result = context.perform_sync(EffectType::IO, "print", &[Value::String("test".to_string())]);
    assert!(io_result.is_ok());
    
    let state_result = context.perform_sync(EffectType::State, "set", &[
    ...
    let random_result = context.perform_sync(EffectType::Random, "int", &[
        Value::Integer(0),
        Value::Integer(100)
    ]);
...
fn test_effect_error_handling() {
    ...
    let result = empty_context.perform_sync(EffectType::IO, "print", &[Value::String("test".to_string())]);
    assert!(result.is_err());
}