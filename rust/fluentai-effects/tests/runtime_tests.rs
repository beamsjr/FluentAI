...
use fluentai_effects::{
    EffectRuntime, EffectContext, EffectHandler, 
...
fn test_context_isolation() {
    ...
    let result = context2.perform_sync(
        EffectType::State,
        "get",
...
fn test_context_handler_override() {
    ...
    // Original handler
    let original_result = context.perform_sync(
        EffectType::Time,
        "now",
        &[],
    ).unwrap();
    ...
    let new_result = context.perform_sync(
        EffectType::Time,
        "now",
        &[],
    ).unwrap();
...
fn test_multiple_operations() {
    ...
    let operations = vec![
        ("set", vec![Value::String("temp".to_string()), Value::Integer(100)]),
        ("get", vec![Value::String("counter".to_string())]),
        ("set", vec![Value::String("counter".to_string()), Value::Integer(5)]),
    ...
    // Verify final state
    let counter = context.perform_sync(
        EffectType::State,
        "get",
...
fn test_effect_composition() {
    ...
    let results = vec![
        ...
        context.perform_sync(EffectType::State, "set", &[
