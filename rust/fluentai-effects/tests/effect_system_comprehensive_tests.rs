...
use fluentai_effects::{
    EffectContext, EffectHandler, EffectRuntime, EffectHandlerProvider,
    IOHandler, StateHandler, ErrorHandler, TimeHandler, RandomHandler,
    NetworkHandler, AsyncHandler, ConcurrentHandler, DomHandler,
};
...
fn test_effect_context_creation_from_provider() {
    ...
    assert!(context.perform_sync(EffectType::IO, "print", &[Value::String("test".to_string())]).is_ok());
    assert!(context.perform_sync(EffectType::State, "get", &[Value::String("key".to_string())]).is_ok());
...
fn test_custom_effect_handler() {
    ...
    assert_eq!(handler.handle_sync("increment", &[]).unwrap(), Value::Integer(1));
    assert_eq!(handler.handle_sync("increment", &[]).unwrap(), Value::Integer(2));
    assert_eq!(handler.handle_sync("decrement", &[]).unwrap(), Value::Integer(1));
...
fn test_random_handler_determinism() {
    ...
    // Set same seed
    handler1.handle_sync("seed", &[Value::Integer(12345)]).unwrap();
    handler2.handle_sync("seed", &[Value::Integer(12345)]).unwrap();
    
    // Generate numbers - should be the same
    for _ in 0..5 {
        let r1 = handler1.handle_sync("int", &[Value::Integer(1), Value::Integer(100)]).unwrap();
        let r2 = handler2.handle_sync("int", &[Value::Integer(1), Value::Integer(100)]).unwrap();
        assert_eq!(r1, r2);
    }
}
