...
fn test_effect_handler_provider_default() {
    ...
    for effect_type in handler_types {
        ...
        assert!(handler.is_ok(), "Handler for {:?} should be available", effect_type);
...
fn test_effect_handler_provider_singleton_behavior() {
    ...
    handler1.handle_sync("set", &[
        Value::String("shared_key".to_string()),
        Value::Integer(42),
    ]).unwrap();
...
async fn test_async_effect_cancellation() {
    ...
    let promise_id = handler.handle_sync("promise", &[Value::String("dummy".to_string())]).unwrap();
...
fn test_effect_context_builder_pattern() {
    ...
    assert!(context.perform_sync(EffectType::Random, "int", &[Value::Integer(1), Value::Integer(10)]).is_err());
}
