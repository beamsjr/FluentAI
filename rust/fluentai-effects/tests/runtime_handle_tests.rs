...
use fluentai_effects::{runtime::RuntimeHandle, EffectRuntime};
use std::sync::{
...
fn test_runtime_handle_enter_guard() {
    ...
    // Test enter guard
    {
        ...
        let result = tokio::task::block_in_place(|| 42);
        assert_eq!(result, 42);
    }
...
fn test_runtime_error_scenarios() {
    ...
    let result = runtime.block_on(async { Result::<i32, String>::Err("Test error".to_string()) });
...
fn test_runtime_handle_multiple_enters() {
    ...
    let _guard1 = handle.enter();
    {
        ...
        let result = tokio::task::block_in_place(|| "nested");
        assert_eq!(result, "nested");
    }
    // Still in outer enter
    let result = tokio::task::block_in_place(|| "outer");
