...
async fn test_dom_handler_create_element() {
    ...
    let result = handler
        .handle_async("create_element", &[Value::String("div".to_string())])
        .await;
    ...
    let result2 = handler
        .handle_async("create_element", &[Value::String("span".to_string())])
        .await;
...
async fn test_dom_handler_create_element_errors() {
    ...
    let result = handler
        .handle_async("create_element", &[Value::Integer(123)])
        .await;
    assert!(result.is_err());
}
...
async fn test_dom_handler_append_child() {
    ...
    let result = handler
        .handle_async("append_child", &[parent_id.clone(), child_id.clone()])
        .await;
...
async fn test_dom_handler_append_child_errors() {
    ...
    let result = handler
        .handle_async(
            "append_child",
            &[Value::String("invalid-parent".to_string()), node_id.clone()],
        )
        .await;
    assert!(result.is_err());
    if let Err(Error::Runtime(msg)) = result {
    ...
    let result = handler
        .handle_async(
            "append_child",
            &[node_id.clone(), Value::String("invalid-child".to_string())],
        )
        .await;
    assert!(result.is_err());
    if let Err(Error::Runtime(msg)) = result {
...
async fn test_dom_handler_remove_child() {
    ...
    let child1_id = handler
        .handle_async("create_element", &[Value::String("span".to_string())])
        .await
        .unwrap();

    let child2_id = handler
        .handle_async("create_element", &[Value::String("p".to_string())])
        .await
        .unwrap();

    // Append both children
    handler
        .handle_async("append_child", &[parent_id.clone(), child1_id.clone()])
        .await
        .unwrap();

    handler
        .handle_async("append_child", &[parent_id.clone(), child2_id.clone()])
        .await
        .unwrap();

    // Remove first child
    let result = handler
        .handle_async("remove_child", &[parent_id.clone(), child1_id.clone()])
        .await;
...
async fn test_dom_handler_remove_child_errors() {
    ...
    let result = handler
        .handle_async("remove_child", &[Value::Integer(123), Value::Integer(456)])
        .await;
    assert!(result.is_err());
}
...
async fn test_dom_handler_set_and_get_attribute() {
    ...
    // Create element
    let node_id = handler
        .handle_async("create_element", &[Value::String("button".to_string())])
        .await
        .unwrap();
    ...
    let result = handler.handle_sync(
        "get_attribute",
        &[node_id.clone(), Value::String("class".to_string())],
    );
    ...
    let result = handler.handle_sync(
        "get_attribute",
        &[node_id.clone(), Value::String("data-id".to_string())],
    );
...
async fn test_dom_handler_set_and_get_text() {
    ...
    let result = handler
        .handle_async(
            "set_text",
            &[node_id.clone(), Value::String("Hello, World!".to_string())],
        )
        .await;
    ...
    handler
        .handle_async(
            "set_text",
            &[node_id.clone(), Value::String("Updated text".to_string())],
        )
        .await
        .unwrap();
...
async fn test_dom_handler_root_operations() {
    ...
    let root_id = handler
        .handle_async("create_element", &[Value::String("html".to_string())])
        .await
        .unwrap();
    ...
    let result = handler
        .handle_async("set_root", &[Value::String("invalid-node".to_string())])
        .await;
    assert!(result.is_err());
}
...
async fn test_dom_handler_query_selector() {
    ...
    // Query for divs
    let result = handler
        .handle_async("query_selector", &[Value::String("div".to_string())])
        .await;
    ...
    let result = handler
        .handle_async("query_selector", &[Value::String("button".to_string())])
        .await;
...
async fn test_dom_handler_query_selector_errors() {
    ...
    let result = handler
        .handle_async("query_selector", &[Value::Integer(123)])
        .await;
    assert!(result.is_err());
}
...
async fn test_dom_handler_serialization() {
    ...
    handler
        .handle_async(
            "set_text",
            &[child.clone(), Value::String("Hello".to_string())],
        )
        .await
        .unwrap();

    handler
        .handle_async("append_child", &[parent.clone(), child.clone()])
        .await
        .unwrap();
...
async fn test_dom_handler_complex_dom_structure() {
    ...
    let body = handler
        .handle_async("create_element", &[Value::String("body".to_string())])
        .await
        .unwrap();
    ...
    // Build structure
    handler
        .handle_async("append_child", &[html.clone(), body.clone()])
        .await
        .unwrap();
    handler
        .handle_async("append_child", &[body.clone(), div1.clone()])
        .await
        .unwrap();
    handler
        .handle_async("append_child", &[body.clone(), div2.clone()])
        .await
        .unwrap();
    handler
        .handle_async("append_child", &[div2.clone(), p1.clone()])
        .await
        .unwrap();
    handler
        .handle_async("append_child", &[div2.clone(), p2.clone()])
        .await
        .unwrap();

    // Set as root
    handler
        .handle_async("set_root", &[html.clone()])
        .await
        .unwrap();

    // Query all paragraphs
    let result = handler
        .handle_async("query_selector", &[Value::String("p".to_string())])
        .await
        .unwrap();
    ...
    handler
        .handle_async("remove_child", &[div2.clone(), p1.clone()])
        .await
        .unwrap();
...
async fn test_dom_handler_add_event_listener_errors() {
    ...
    let result = handler
        .handle_async(
            "add_event_listener",
            &[node.clone(), Value::String("click".to_string())],
        )
        .await;
    assert!(result.is_err());
    if let Err(Error::Runtime(msg)) = result {
...
async fn test_dom_handler_multiple_event_types() {
    ...
    let element = handler
        .handle_async("create_element", &[Value::String("input".to_string())])
        .await
        .unwrap();
...
async fn test_dom_handler_remove_element() {
    ...
    handler
        .handle_async("remove_child", &[parent.clone(), child.clone()])
        .await
        .unwrap();
...
fn test_dom_handler_is_async_operation() {
    ...
    let sync_ops = vec!["get_attribute", "get_text", "serialize"];
