...
fn test_io_handler_file_operations() {
    ...
    // Test write_file
    let write_result = handler.handle_sync("write_file", &[
    ...
    // Test read_file
    let read_result = handler.handle_sync("read_file", &[
        Value::String(test_file.to_string()),
    ]);
...
fn test_state_handler_get_set() {
    ...
    // Test set operation
    let set_result = handler.handle_sync("set", &[
...
fn test_state_handler_delete() {
    ...
    // Delete it
    let delete_result = handler.handle_sync("delete", &[
        Value::String("temp".to_string()),
    ]);
...
fn test_error_handler_throw_catch() {
    ...
    let raise_result = handler.handle_sync("raise", &[
...
fn test_error_handler_try_operations() {
    ...
    let try_result = handler.handle_sync("try", &[
...
fn test_time_handler_now() {
    ...
    if let Value::Integer(timestamp) = now_result.unwrap() {
        ...
        assert!(time > SystemTime::UNIX_EPOCH + Duration::from_secs(1577836800)); // Jan 1, 2020
    } else {
...
fn test_time_handler_format() {
    ...
    let format_result = handler.handle_sync("format", &[
        Value::Integer(SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_millis() as i64),
...
fn test_time_handler_sleep() {
    ...
    let sleep_result = handler.handle_sync("sleep", &[
...
fn test_random_handler_choice() {
    ...
    let result = handler.handle_sync("choice", &[
...
fn test_random_handler_seed() {
    ...
    // Test setting seed
    let seed_result = handler.handle_sync("seed", &[
        Value::Integer(12345),
    ]);
    assert!(seed_result.is_ok());
    
    // Generate some random numbers
    let r1 = handler.handle_sync("int", &[
        Value::Integer(1),
        Value::Integer(1000),
    ]).unwrap();
    
    // Reset seed to same value
    handler.handle_sync("seed", &[
        Value::Integer(12345),
    ]).unwrap();
    
    // Should get same random number
    let r2 = handler.handle_sync("int", &[
        Value::Integer(1),
        Value::Integer(1000),
    ]).unwrap();
...
fn test_network_handler_operations() {
    ...
    let result = handler.handle_sync("fetch", &[
...
fn test_concurrent_handler_mutex() {
    ...
    // Test mutex creation
    let mutex_result = handler.handle_sync("mutex", &[
...
fn test_dom_handler_operations() {
    ...
    let ops = ["get_element", "create_element", "set_attribute", "add_event_listener"];
...
fn test_handler_error_propagation() {
    ...
    let result = context.perform_sync(EffectType::IO, "read_file", &[
...
fn test_multiple_handlers_isolation() {
    ...
    handler1.handle_sync("set", &[
        Value::String("key".to_string()),
        Value::Integer(1),
    ]).unwrap();
    
    // Second handler should have its own state
    let result = handler2.handle_sync("get", &[
        Value::String("key".to_string()),
    ]).unwrap();
