    ...
    fn handle_sync(&self, operation: &str, args: &[Value]) -> EffectResult {
        match operation {
            "promise" => {
                ...
                self.promises
                    .insert(promise_id.clone(), Arc::new(Mutex::new(rx)));
            ...
            "resolve" => {
                if args.len() >= 2 {
                    if let Some(Value::String(_promise_id)) = args.first() {
                        ...
                        Err(Error::Runtime(
                            "async:resolve requires promise ID".to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "async:resolve requires 2 arguments".to_string(),
                    ))
                }
            }
            "reject" => {
                if args.len() >= 2 {
                    if let Some(Value::String(_promise_id)) = args.first() {
                        ...
                        Err(Error::Runtime(
                            "async:reject requires promise ID".to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "async:reject requires 2 arguments".to_string(),
                    ))
                }
            }
        ...
        match operation {
            "await" => {
                if let Some(Value::String(promise_id)) = args.first() {
                    ...
                    Err(Error::Runtime(
                        "async:await requires promise ID".to_string(),
                    ))
                }
            }
            "all" => {
