    ...
    fn handle_sync(&self, operation: &str, args: &[Value]) -> EffectResult {
        match operation {
            "get_attribute" => {
                if args.len() >= 2 {
                        ...
                        (args.get(0), args.get(1))
                    {
                        ...
                        Err(Error::Runtime(
                            "dom:get_attribute requires node ID and attribute name".to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "dom:get_attribute requires 2 arguments".to_string(),
                    ))
                }
            }
            "get_text" => {
            ...
            "serialize" => {
                ...
                let nodes: std::collections::HashMap<String, serde_json::Value> = self
                    .nodes
                    .iter()
                    .map(|entry| {
        ...
        match operation {
            "create_element" => {
                if let Some(Value::String(tag)) = args.first() {
                    ...
                    Err(Error::Runtime(
                        "dom:create_element requires tag name".to_string(),
                    ))
                }
            }
            "set_attribute" => {
                if args.len() >= 3 {
                    if let (Some(Value::String(node_id)), Some(Value::String(attr)), Some(value)) =
                        (args.get(0), args.get(1), args.get(2))
                    {
                        ...
                        Err(Error::Runtime(
                            "dom:set_attribute requires node ID, attribute name, and value"
                                .to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "dom:set_attribute requires 3 arguments".to_string(),
                    ))
                }
            }
            "set_text" => {
                if args.len() >= 2 {
                    if let (Some(Value::String(node_id)), Some(text)) = (args.get(0), args.get(1)) {
                        if let Some(mut node) = self.nodes.get_mut(node_id) {
                        ...
                        Err(Error::Runtime(
                            "dom:set_text requires node ID and text".to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "dom:set_text requires 2 arguments".to_string(),
                    ))
                }
            }
            "append_child" => {
                if args.len() >= 2 {
                        ...
                        (args.get(0), args.get(1))
                    {
                        ...
                        Err(Error::Runtime(
                            "dom:append_child requires parent and child node IDs".to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "dom:append_child requires 2 arguments".to_string(),
                    ))
                }
            }
            "remove_child" => {
                if args.len() >= 2 {
                        ...
                        (args.get(0), args.get(1))
                    {
                        ...
                        Err(Error::Runtime(
                            "dom:remove_child requires parent and child node IDs".to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "dom:remove_child requires 2 arguments".to_string(),
                    ))
                }
            }
            "set_root" => {
            ...
            "query_selector" => {
                if let Some(Value::String(selector)) = args.first() {
                    ...
                    Ok(Value::List(
                        matching_nodes.into_iter().map(Value::String).collect(),
                    ))
                } else {
                    Err(Error::Runtime(
                        "dom:query_selector requires selector".to_string(),
                    ))
                }
            }
            "add_event_listener" => {
                if args.len() >= 3 {
                    ...
                    ) = (args.get(0), args.get(1), args.get(2))
                    {
                        ...
                        Err(Error::Runtime(
                            "dom:add_event_listener requires node ID, event type, and handler ID"
                                .to_string(),
                        ))
                    }
                } else {
                    Err(Error::Runtime(
                        "dom:add_event_listener requires 3 arguments".to_string(),
                    ))
                }
            }
            _ => Err(Error::Runtime(format!(
                "Unknown DOM operation: {}",
                operation
            ))),
        }
    }
