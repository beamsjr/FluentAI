...
use crate::{EffectContext, EffectHandler, EffectType};
...
impl EffectHandlerProvider {
        ...
        F: Fn() -> Arc<dyn EffectHandler> + Send + Sync + 'static,
    {
        self.factories
            .write()
            .insert(effect_type, Box::new(factory));
    }
    ...
    pub fn get_handler(&self, effect_type: EffectType) -> Result<Arc<dyn EffectHandler>> {
        ...
        if let Some(factory) = self.factories.read().get(&effect_type) {
            ...
            self.instances
                .write()
                .insert(effect_type, Arc::clone(&handler));
            return Ok(handler);
        }
...
pub mod di {
    ...
    pub fn register_effect_services(builder: &mut ContainerBuilder) -> Result<()> {
        ...
        builder
            .register_singleton(|| Arc::new(EffectHandlerBuilder::new().with_defaults().build()));
...
mod tests {
    ...
    fn test_provider_registration() {
        ...
        // Register factory
        provider.register_factory(EffectType::State, || Arc::new(TestHandler::new("factory")));
