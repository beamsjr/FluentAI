...
fn main() {
    ...
    let test_cases = vec![
        ...
        (r#"private function create_stock(symbol, shares) { {"symbol": symbol, "shares": shares} }"#, "function with map"),
        ...
        // F-string printing
        (r#"$(f"Portfolio Value: ${value}").print()"#, "f-string with print"),
        
        // Object field access
        (r#"stock.symbol"#, "nested field access"),
        
        // Conditionals
        (r#"if (percentage < 0.0) { false } else { true }"#, "if-else expression"),
        
        // Array push
        (r#"stocks.push(new_stock)"#, "array push method"),
        
        // Array reduce
        (r#"stocks.reduce(0.0, (sum, stock) => sum + stock.value)"#, "array reduce"),
