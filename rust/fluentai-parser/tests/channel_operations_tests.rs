...
fn test_parse_channel_communication_pattern() {
    ...
    match node {
        Node::Let { bindings, body } => {
            ...
            assert_eq!(bindings[0].0, "ch");
            assert!(matches!(result.get_node(bindings[0].1).unwrap(), Node::Channel));
...
fn test_parse_multiple_channels() {
    ...
    match node {
        Node::Let { bindings, .. } => {
            ...
            assert!(matches!(result.get_node(bindings[0].1).unwrap(), Node::Channel));
            assert!(matches!(result.get_node(bindings[1].1).unwrap(), Node::Channel));
        }
        _ => panic!("Expected Let node"),
    }
}
...
fn test_parse_channel_pipeline() {
    ...
    let result = parse(input);
    assert!(result.is_ok(), "Failed to parse channel pipeline: {:?}", result);
}