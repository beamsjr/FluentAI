            ...
            if let Some(node) = graph.get_node(root_id) {
                match node {
                    Node::Application { function, args } => {
                        ...
                        if let Some(Node::Application { function: to_string_fn, args: to_string_args }) = graph.get_node(args[1]) {
                        ...
                        if let Some(Node::Application { function: inner_append, args: inner_args }) = graph.get_node(args[1]) {
                            ...
                            if let Some(Node::Application { function: to_string_fn, args: to_string_args }) = graph.get_node(inner_args[0]) {
                                if let Some(Node::Variable { name }) = graph.get_node(*to_string_fn) {
                                    assert_eq!(name, "to_string");
                                }
                                assert_eq!(to_string_args.len(), 1);
                            ...
                            if let Some(Node::Literal(Literal::String(s))) = graph.get_node(inner_args[1]) {
                                println!("Last part: {:?}", s);
                                assert_eq!(s, "!");
                            }
                        }
