...
mod test_fstring_complex {
            ...
            if let Node::Application { function, args } = node {
                if let Some(Node::Variable { name }) = graph.get_node(*function) {
                    ...
                    if name == "+" && args.len() == 2 {
                        ...
                        if let (Some(Node::Variable { name: n1 }), Some(Node::Variable { name: n2 })) = 
    ...
    fn test_fstring_with_multiple_interpolations() {
        ...
        assert!(property_accesses >= 2, "Should find at least 2 property accesses, found {}", property_accesses);
    }
    ...
    fn test_fstring_with_escaped_braces() {
        ...
        if let Some(root_id) = graph.root_id {
            if let Some(Node::Literal(fluentai_core::ast::Literal::String(s))) = graph.get_node(root_id) {
                assert_eq!(s, "Value: {x}");
