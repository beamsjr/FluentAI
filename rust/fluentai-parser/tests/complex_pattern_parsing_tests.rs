...
use fluentai_core::ast::{Node, Pattern, Literal, RangePattern};
    ...
    if let Some(Node::Match { branches, .. }) = graph.get_node(root) {
        ...
        match &branches[0].0 {
            Pattern::Or(patterns) => {
                ...
                assert!(matches!(&patterns[2], Pattern::Literal(Literal::Integer(3))));
            }
    ...
    if let Some(Node::Match { branches, .. }) = graph.get_node(root) {
        ...
        match &branches[0].0 {
            Pattern::As { binding, pattern } => {
                ...
                assert!(matches!(pattern.as_ref(), Pattern::Constructor { name, .. } if name == "Cons"));
            }
    ...
    if let Some(Node::Match { branches, .. }) = graph.get_node(root) {
        match &branches[0].0 {
            Pattern::View { function: _, pattern } => {
                assert!(matches!(pattern.as_ref(), Pattern::Literal(Literal::Integer(5))));
            }
    ...
    if let Some(Node::Match { branches, .. }) = graph.get_node(root) {
        ...
        match &branches[0].0 {
            Pattern::Or(patterns) => {
                ...
                assert!(matches!(&patterns[0], Pattern::Literal(Literal::String(s)) if s == "Saturday"));
                assert!(matches!(&patterns[1], Pattern::Literal(Literal::String(s)) if s == "Sunday"));
            }
