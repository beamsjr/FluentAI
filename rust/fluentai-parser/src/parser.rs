...
impl<'a> Parser<'a> {
    ...
    fn parse_effect_inner(&mut self) -> ParseResult<NodeId> {
        ...
        if let Some(Token::Symbol(effect_spec)) = self.lexer.next_token() {
            ...
            Err(ParseError::InvalidSyntax(
                "Expected effect specification".to_string(),
            ))
        }
    }
    ...
    fn parse_handler_inner(&mut self) -> ParseResult<NodeId> {
        ...
        while matches!(self.lexer.peek_token(), Some(Token::LParen)) {
            ...
            if let Some(Token::Symbol(effect_spec)) = self.lexer.next_token() {
                ...
                return Err(ParseError::InvalidSyntax(
                    "Expected effect type in handler".to_string(),
                ));
            }
        }
    ...
    fn parse_pattern_inner(&mut self) -> ParseResult<Pattern> {
                    ...
                    match keyword_str.as_ref() {
                        ...
                        _ => {
                            ...
                            if !keyword_str.is_empty()
                                && keyword_str.chars().next().unwrap().is_uppercase()
                            {
        ...
        match self.lexer.peek_token().cloned() {
                ...
                if !name.is_empty() && name.chars().next().unwrap().is_uppercase() =>
            {
                ...
                // Parse sub-patterns - only if they look like patterns
                // Stop if we see something that's clearly not a sub-pattern
                loop {
                    match self.lexer.peek_token() {
                        Some(Token::RParen) | Some(Token::LParen) => break,
                        Some(Token::String(_))
                        | Some(Token::Integer(_))
                        | Some(Token::Float(_)) => {
                            // These could be the body of the match branch, not sub-patterns
                            // Only parse them if we haven't parsed any sub-patterns yet
                        ...
                        {
                            // This looks like a pattern variable or constructor
                            patterns.push(self.parse_pattern()?);
                        }
                        _ => break,
                    }
    ...
    fn parse_as_pattern(&mut self) -> ParseResult<Pattern> {
        ...
        let binding = if let Some(Token::Symbol(name)) = self.lexer.next_token() {
            ...
            return Err(ParseError::InvalidSyntax(
                "Expected variable name in as-pattern".to_string(),
            ));
        };
    ...
    fn parse_qualified_symbol(&mut self) -> ParseResult<NodeId> {
        if let Some(Token::QualifiedSymbol(qualified)) = self.lexer.next_token() {
            ...
            Err(ParseError::InvalidSyntax(
                "Expected qualified symbol".to_string(),
            ))
        }
    }
                ...
                Some(Token::Symbol("export")) => {
                    ...
                    while !matches!(self.lexer.peek_token(), Some(Token::RParen)) {
                        if let Some(Token::Symbol(export_name)) = self.lexer.next_token() {
                            ...
                            return Err(ParseError::InvalidSyntax(
                                "Expected export name".to_string(),
                            ));
                        }
                    }
                    self.expect_token(Token::RParen)?;
        ...
        if matches!(self.lexer.peek_token(), Some(Token::Symbol("*"))) {
            ...
            while !matches!(self.lexer.peek_token(), Some(Token::RParen)) {
                if let Some(Token::Symbol(name)) = self.lexer.next_token() {
                    ...
                    return Err(ParseError::InvalidSyntax(
                        "Expected import name".to_string(),
                    ));
                }
            }
            self.expect_token(Token::RParen)?;
    ...
    fn parse_contract_inner(&mut self) -> ParseResult<NodeId> {
        ...
        let function_name = match self.lexer.next_token() {
            ...
            _ => {
                return Err(ParseError::InvalidSyntax(
                    "Expected function name in contract".to_string(),
                ))
            }
        };
        ...
        while !matches!(self.lexer.peek_token(), Some(Token::RParen)) {
            match self.lexer.next_token() {
                    ...
                    match keyword {
                        ...
                        ":complexity" => {
                            ...
                            match self.lexer.next_token() {
                                ...
                                _ => {
                                    return Err(ParseError::InvalidSyntax(
                                        "Expected string for complexity".to_string(),
                                    ))
                                }
                            }
                        }
                        ":pure" => {
                            ...
                            match self.lexer.next_token() {
                                ...
                                _ => {
                                    return Err(ParseError::InvalidSyntax(
                                        "Expected boolean for :pure".to_string(),
                                    ))
                                }
                            }
                        }
                ...
                _ => {
                    return Err(ParseError::InvalidSyntax(
                        "Expected contract keyword".to_string(),
                    ))
                }
            }
        }
