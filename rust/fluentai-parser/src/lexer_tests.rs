...
mod tests {
    ...
    fn test_string_escapes() {
        ...
        assert_eq!(
            lexer.next_token(),
            Some(Token::String("tab\there".to_string()))
        );
        assert_eq!(
            lexer.next_token(),
            Some(Token::String("quote\"test".to_string()))
        );
        assert_eq!(
            lexer.next_token(),
            Some(Token::String("slash\\test".to_string()))
        );
        assert_eq!(
            lexer.next_token(),
            Some(Token::String("return\rtest".to_string()))
        );
        assert_eq!(lexer.next_token(), None);
    }
    ...
    fn test_string_unknown_escape() {
        ...
        assert_eq!(
            lexer.next_token(),
            Some(Token::String("hello\\xworld".to_string()))
        );
    }
    ...
    fn test_string_with_spaces() {
        ...
        assert_eq!(
            lexer.next_token(),
            Some(Token::String("hello world with spaces".to_string()))
        );
    }
    ...
    fn test_qualified_symbols() {
        ...
        assert_eq!(
            lexer.next_token(),
            Some(Token::QualifiedSymbol("std.print"))
        );
        assert_eq!(lexer.next_token(), Some(Token::QualifiedSymbol("math.sin")));
        assert_eq!(
            lexer.next_token(),
            Some(Token::QualifiedSymbol("module.export"))
        );
        assert_eq!(lexer.next_token(), None);
    }
    ...
    fn test_qualified_symbols_with_underscores() {
        ...
        assert_eq!(
            lexer.next_token(),
            Some(Token::QualifiedSymbol("my_module.my_function"))
        );
        assert_eq!(
            lexer.next_token(),
            Some(Token::QualifiedSymbol("test_lib.helper_fn"))
        );
        assert_eq!(lexer.next_token(), None);
    }
    ...
    fn test_complex_expression() {
        let mut lexer =
            Lexer::new("(define (factorial n) (if (<= n 1) 1 (* n (factorial (- n 1)))))");
        let expected = vec![
            Token::LParen,
            Token::Symbol("define"),
            Token::LParen,
    ...
    fn test_unicode_in_strings() {
        ...
        assert_eq!(
            lexer.next_token(),
            Some(Token::String("Hello, 世界".to_string()))
        );
        assert_eq!(lexer.next_token(), Some(Token::String("🦀".to_string())));
        assert_eq!(lexer.next_token(), None);
    }
