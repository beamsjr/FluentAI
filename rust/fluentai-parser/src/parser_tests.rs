...
mod tests {
    ...
    use fluentai_core::ast::{Node, Literal, Pattern, EffectType};
    ...
    fn test_parse_integers() {
        assert!(matches!(parse_single_expr("42").unwrap(), 
            Node::Literal(Literal::Integer(42))));
        assert!(matches!(parse_single_expr("-123").unwrap(), 
            Node::Literal(Literal::Integer(-123))));
        assert!(matches!(parse_single_expr("0").unwrap(), 
    ...
    fn test_parse_booleans() {
        assert!(matches!(parse_single_expr("true").unwrap(), 
            Node::Literal(Literal::Boolean(true))));
        assert!(matches!(parse_single_expr("false").unwrap(), 
            Node::Literal(Literal::Boolean(false))));
        assert!(matches!(parse_single_expr("#t").unwrap(), 
            Node::Literal(Literal::Boolean(true))));
        assert!(matches!(parse_single_expr("#f").unwrap(), 
            Node::Literal(Literal::Boolean(false))));
    }
    ...
    fn test_parse_multi_param_lambda() {
        ...
        match result.get_node(root_id).unwrap() {
            Node::Lambda { params, body } => {
                ...
                assert!(matches!(result.get_node(*body).unwrap(), Node::Application { .. }));
            }
            _ => panic!("Expected Lambda node"),
        }
    }
    ...
    fn test_parse_simple_let() {
        ...
        match result.get_node(root_id).unwrap() {
            Node::Let { bindings, body } => {
                ...
                assert!(matches!(result.get_node(bindings[0].1).unwrap(), 
                    Node::Literal(Literal::Integer(42))));
                assert!(matches!(result.get_node(*body).unwrap(), 
                    Node::Variable { name } if name == "x"));
            }
    ...
    fn test_parse_letrec() {
        let result = parse("(letrec ((fact (lambda (n) (if (= n 0) 1 (* n (fact (- n 1))))))) (fact 5))").unwrap();
        let root_id = result.root_id.unwrap();
        assert!(matches!(result.get_node(root_id).unwrap(), Node::Letrec { .. }));
    }
    ...
    fn test_parse_if() {
        ...
        match result.get_node(root_id).unwrap() {
            Node::If { condition, then_branch, else_branch } => {
                assert!(matches!(result.get_node(*condition).unwrap(), Node::Application { .. }));
        ...
        match result.get_node(root_id).unwrap() {
            Node::Application { function, args } => {
                assert!(matches!(result.get_node(*function).unwrap(), 
                    Node::Variable { name } if name == "+"));
                assert_eq!(args.len(), 2);
                assert!(matches!(result.get_node(args[0]).unwrap(), Node::Application { .. }));
                assert!(matches!(result.get_node(args[1]).unwrap(), Node::Application { .. }));
            }
            _ => panic!("Expected Application node"),
        }
    }
    ...
    fn test_parse_simple_match() {
        ...
        match result.get_node(root_id).unwrap() {
            Node::Match { expr, branches } => {
                ...
                assert!(matches!(&branches[0].0, Pattern::Literal(Literal::Integer(0))));
                assert!(matches!(&branches[1].0, Pattern::Literal(Literal::Integer(1))));
    ...
    fn test_parse_async() {
        ...
        let root_id = result.root_id.unwrap();
        assert!(matches!(result.get_node(root_id).unwrap(), Node::Async { .. }));
    }
    ...
    fn test_parse_await() {
        ...
        let root_id = result.root_id.unwrap();
        assert!(matches!(result.get_node(root_id).unwrap(), Node::Await { .. }));
    }
    ...
    fn test_parse_spawn() {
        ...
        let root_id = result.root_id.unwrap();
        assert!(matches!(result.get_node(root_id).unwrap(), Node::Spawn { .. }));
    }
    ...
    fn test_parse_send() {
        ...
        let root_id = result.root_id.unwrap();
        assert!(matches!(result.get_node(root_id).unwrap(), Node::Send { .. }));
    }
    ...
    fn test_parse_receive() {
        ...
        let root_id = result.root_id.unwrap();
        assert!(matches!(result.get_node(root_id).unwrap(), Node::Receive { .. }));
    }
        ...
        match result.get_node(root_id).unwrap() {
            Node::Let { bindings, body } => {
                ...
                match result.get_node(*body).unwrap() {
                    Node::Let { bindings: inner_bindings, body: inner_body } => {
                        assert_eq!(inner_bindings.len(), 1);
                        assert_eq!(inner_bindings[0].0, "_");
                        // Final expression should be 42
                        assert!(matches!(result.get_node(*inner_body).unwrap(), 
                            Node::Literal(Literal::Integer(42))));
                    }
    ...
    fn test_parse_unicode_symbols() {
        ...
        assert!(matches!(graph.get_node(root_id).unwrap(), Node::Application { .. }));
    }
}