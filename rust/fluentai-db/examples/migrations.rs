...
async fn main() -> anyhow::Result<()> {
    println!("=== FluentAi Database Migration Example ===\n");
    ...
    println!(
        ...
        rollback_result.migrations_run.len()
    );
...
fn create_migrations(runner: &mut MigrationRunner) {
    ...
    let migration_004 = Arc::new(
        ...
        VALUES
            (1, 'Welcome to FluentAi', 'This is the first post!', 1, datetime('now')),
...
async fn show_status(runner: &MigrationRunner) -> anyhow::Result<()> {
    ...
    for s in status {
        match s {
            fluentai_db::migration::runner::MigrationStatus::Pending { migration } => {
                println!(
                    "  [ ] {} - {} (pending)",
                    migration.version(),
                    migration.name()
            ...
            fluentai_db::migration::runner::MigrationStatus::Applied {
                metadata,
                checksum_valid,
            } => {
                ...
                println!(
                    "  [✓] {} - {} ({} checksum)",
                    metadata.version, metadata.name, checksum_status
                );
            }
            fluentai_db::migration::runner::MigrationStatus::Missing { metadata } => {
                println!("  [?] {} - {} (missing)", metadata.version, metadata.name);
            }
        }
    }
...
async fn verify_schema(connection: Arc<DbConnection>) -> anyhow::Result<()> {
    // Check users table
    let users_count = connection
        .fetch_all("SELECT COUNT(*) as count FROM users", vec![])
        .await?;
    println!("  ✓ Users table exists with {} records", users_count.len());

    // Check posts table
    let posts_count = connection
        .fetch_all("SELECT COUNT(*) as count FROM posts", vec![])
        .await?;
    println!("  ✓ Posts table exists with {} records", posts_count.len());

    // Show sample data
    let users = connection
        .fetch_all("SELECT username, email, active FROM users", vec![])
        .await?;
