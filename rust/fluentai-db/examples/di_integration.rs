...
use anyhow::Result;
use fluentai_db::{
    with_transaction, ContainerDatabaseProvider, DatabaseContainerBuilderExt,
    DatabaseServiceProvider, DatabaseServicesBuilder, DbConfig,
};
use fluentai_di::prelude::*;
use fluentai_effects::{EffectHandlerBuilder, EffectHandlerProvider};
use std::sync::Arc;
...
async fn method1_extension_trait() -> Result<()> {
    ...
    conn.execute_raw_unsafe("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT NOT NULL)")
        .await?;
    ...
    println!(
        "✓ Got transaction manager, active transactions: {}",
...
async fn method2_services_builder() -> Result<()> {
    ...
    // Verify data
    let rows = conn
        .fetch_all("SELECT COUNT(*) as count FROM products", vec![])
        .await?;
...
async fn method3_complete_integration() -> Result<()> {
    ...
    builder.register_singleton(|| Arc::new(EffectHandlerBuilder::new().with_defaults().build()));
...
impl ApplicationService {
    ...
    async fn run(&self) -> Result<()> {
        ...
        if let Ok(effect_provider) = self.container.resolve::<Arc<EffectHandlerProvider>>() {
            ...
            let result = context.perform_sync(EffectType::IO, "db:is-connected", &[])?;
