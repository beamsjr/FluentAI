...
use anyhow::Result;
use fluentai_db::{
    DatabaseContainerBuilderExt, DatabaseServiceProvider, DatabaseServicesBuilder, DbConfig,
};
...
fn method1_extension_trait() -> Result<()> {
    ...
    println!(
        "✓ Got database config from DI container: {}",
        registered_config.url
    );
...
impl DatabaseServiceProvider for MockDatabaseProvider {
    fn get_connection(&self) -> fluentai_db::error::DbResult<Arc<fluentai_db::DbConnection>> {
        Err(fluentai_db::error::DbError::Connection(
            "Mock provider - no real connection".into(),
        ))
    }

    fn get_transaction_manager(
        &self,
    ) -> fluentai_db::error::DbResult<Arc<fluentai_db::TransactionManager>> {
        Err(fluentai_db::error::DbError::Connection(
            "Mock provider - no real transaction manager".into(),
        ))
    }

    fn get_migration_runner(
        &self,
    ) -> fluentai_db::error::DbResult<Arc<fluentai_db::MigrationRunner>> {
        Err(fluentai_db::error::DbError::Connection(
            "Mock provider - no real migration runner".into(),
        ))
    }
}
