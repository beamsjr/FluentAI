...
use fluentai_db::{
    with_transaction, with_transaction_retry, DbConfig, DbConnection, DbResult, IsolationLevel,
...
async fn basic_transaction_example(conn: Arc<DbConnection>) -> DbResult<()> {
    ...
    // Check balances
    let rows = tx
        .fetch_all("SELECT name, balance FROM accounts ORDER BY name", vec![])
        .await?;
...
async fn transaction_options_example(conn: Arc<DbConnection>) -> DbResult<()> {
    ...
    println!(
        "  Read {} accounts with READ COMMITTED isolation",
        rows.len()
    );
