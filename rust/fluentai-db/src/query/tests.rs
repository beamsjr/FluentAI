...
mod tests {
    ...
    #[test]
    fn test_join_clause() {
        let join_condition = eq(col("users.id"), col("orders.user_id"));

        let mut query = Query::new();
        query.from = Some(QueryExpr::Table("users".to_string()));
        query.select = vec![(col("users.name"), None), (col("orders.total"), None)];
        query.joins = vec![(
            JoinType::Inner,
            QueryExpr::Table("orders".to_string()),
            join_condition,
        )];
    ...
    fn test_unary_operators() {
        ...
        let where_clause = and(not_expr, and(is_null_expr, is_not_null_expr));
