...
use crate::schema::{FieldType, Schema, SqlDialect};
use std::sync::Arc;
...
impl MigrationBuilder {
    ...
    pub fn create_table(mut self, name: impl Into<String>, schema: Schema) -> Self {
        ...
        self.down_operations
            .insert(0, MigrationOperation::DropTable { name: table_name });
        self
    }
    ...
    pub fn create_index(
        ...
        self.down_operations
            .insert(0, MigrationOperation::DropIndex { name: index_name });
    ...
    pub fn up_sql(mut self, sql: impl Into<String>) -> Self {
        self.up_operations
            .push(MigrationOperation::Raw { sql: sql.into() });
        self
    }
    ...
    pub fn build(self) -> Arc<dyn Migration> {
        ...
        let mut migration = SqlMigration::new(self.version, self.name, up_sql, down_sql);
        ...
        match operation {
                ...
                let columns = schema
                    ...
                    .map(|field| {
                        let mut sql = format!(
                            "{} {}",
                            field.name,
                            field.field_type.to_sql(SqlDialect::SQLite)
                        );
            ...
            MigrationOperation::AddColumn {
                ...
                let mut sql = format!(
                    ...
                    table,
                    column,
                    field_type.to_sql(SqlDialect::SQLite)
                );
