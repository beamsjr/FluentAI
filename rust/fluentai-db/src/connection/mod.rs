...
impl DbConnection {
    ...
    pub async fn execute_raw_unsafe(&self, query: &str) -> DbResult<u64> {
        eprintln!(
            "WARNING: Using raw SQL execution without parameters. Ensure this query is safe!"
        );
...
pub fn register_db_services(
    builder: &mut fluentai_di::builder::ContainerBuilder,
    config: DbConfig,
) -> DbResult<()> {
    ...
    builder.register_singleton(move || ConnectionPool::new(config1.clone()));

    // Register connection service
    builder.register_singleton(move || DbConnectionService::new(config2.clone()));
...
impl<'a> Drop for Transaction<'a> {
    fn drop(&mut self) {
        if self.tx.is_some() && !self.committed {
            ...
            eprintln!(
                "Warning: Transaction dropped without explicit commit or rollback - rolling back"
            );
        }
    }
}
