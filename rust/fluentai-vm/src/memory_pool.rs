...
impl SlabAllocator {
    ...
    pub fn allocate(&mut self, size: usize, align: usize) -> Result<*mut u8> {
        if size > self.config.slab_size {
            return Err(anyhow!("Allocation size {} exceeds slab size {}", size, self.config.slab_size));
        }
...
impl MemoryPool {
    ...
    pub fn stats(&self) -> PoolStats {
        ...
        PoolStats {
            ...
            deallocations: slab_stats.deallocations + string_stats.deallocations + vec_stats.deallocations,
            current_used: slab_stats.current_used + string_stats.current_used + vec_stats.current_used,
            peak_used: slab_stats.peak_used.max(string_stats.peak_used).max(vec_stats.peak_used),
            failed_allocations: slab_stats.failed_allocations + string_stats.failed_allocations + vec_stats.failed_allocations,
        }
    }
}
