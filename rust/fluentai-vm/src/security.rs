...
impl SecurityContext {
    ...
    pub fn check_module_import(&self, module: &str) -> Result<()> {
        ...
        // Check capability
        let cap = Capability::ModuleImport {
...
impl TaintTracker {
    ...
    pub fn check_taint_flow(&self, from_id: u64, to_context: &str) -> Result<()> {
        let taint = self.get_taint(from_id);
        match taint {
            TaintLevel::Sensitive => Err(anyhow!("Cannot flow sensitive data to {}", to_context)),
...
impl ModuleIsolation {
    ...
    pub fn create_module_context(&self, module: &str, policy: SecurityPolicy) {
        self.module_globals
            .write()
            .unwrap()
            .insert(module.to_string(), FxHashMap::default());
        self.module_capabilities
            .write()
            .unwrap()
            .insert(module.to_string(), FxHashSet::default());
        self.module_limits
            .write()
            .unwrap()
            .insert(module.to_string(), policy);
    }
