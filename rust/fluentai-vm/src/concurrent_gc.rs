...
impl ConcurrentGc {
    ...
    fn trigger_minor_gc(&self) -> Result<()> {
        ...
        self.phase.store(GcPhase::Marking as usize, Ordering::SeqCst);
        ...
        self.phase.store(GcPhase::Sweeping as usize, Ordering::SeqCst);
        self.sweep_and_promote_young()?;
        
        self.phase.store(GcPhase::Idle as usize, Ordering::SeqCst);
        
        let elapsed = start.elapsed();
        self.stats.total_pause_ns.fetch_add(elapsed.as_nanos() as usize, Ordering::Relaxed);
    ...
    fn trigger_major_gc(&self) -> Result<()> {
        ...
        // Optional compaction
        self.phase.store(GcPhase::Compacting as usize, Ordering::SeqCst);
