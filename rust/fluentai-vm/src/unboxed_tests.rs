...
mod tests {
    ...
    mod boxed_values {
        ...
        fn test_promise_boxed() {
            let val =
                UnboxedValue::Boxed(Box::new(BoxedValue::Promise(crate::safety::PromiseId(456))));
            assert!(val.is_truthy());
        ...
        fn test_channel_boxed() {
            let val =
                UnboxedValue::Boxed(Box::new(BoxedValue::Channel(crate::safety::ChannelId(789))));
            assert!(val.is_truthy());
    ...
    mod value_conversion {
        ...
        fn test_from_value_primitives() {
            assert!(matches!(
                UnboxedValue::from_value(Value::Nil),
                UnboxedValue::Nil
            ));
            assert!(matches!(
                UnboxedValue::from_value(Value::Boolean(true)),
                UnboxedValue::Bool(true)
            ));
            assert!(matches!(
                UnboxedValue::from_value(Value::Integer(42)),
                UnboxedValue::Int(42)
            ));
            assert!(
                matches!(UnboxedValue::from_value(Value::Float(3.14)), UnboxedValue::Float(f) if f == 3.14)
            );
        }
        ...
        fn test_from_value_list() {
            let val =
                UnboxedValue::from_value(Value::List(vec![Value::Integer(1), Value::Integer(2)]));
            match val {
                UnboxedValue::Boxed(boxed) => match &*boxed {
            ...
            for val in values {
                ...
                assert!(
                    val.eq(&cloned)
                        || matches!(
                            (val, cloned),
                            (UnboxedValue::Float(_), UnboxedValue::Float(_))
                        )
                );
            }
        }
