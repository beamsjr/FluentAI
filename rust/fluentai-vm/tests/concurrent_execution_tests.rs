            ...
            s.spawn(move |_| {
                ...
                while consumed < 100 && retries < 10000 {
                    if queue_clone.try_pop().is_some() {
                        ...
                        if consumed > 0 && success_clone.load(Ordering::Relaxed) >= num_threads/2 * 100 {
...
fn test_work_stealing_deque() {
    ...
    let owner_handle = thread::spawn(move || {
        for i in 0..500 { // Reduce to 500 items to avoid overflow
...
fn test_concurrent_structure_fairness() {
    ...
    if zero_work_threads > 0 {
        println!("Warning: {} threads got no work (this is normal for lock-free queues)", zero_work_threads);
    }
