...
fn test_compile_module_with_exports() -> Result<()> {
    ...
    let a = graph.add_node(Node::Literal(Literal::Integer(1))).expect("Failed to add node");
    let b = graph.add_node(Node::Literal(Literal::Integer(2))).expect("Failed to add node");
    let c = graph.add_node(Node::Literal(Literal::Integer(3))).expect("Failed to add node");
    ...
    compile_and_check_opcodes(&graph, &[Opcode::BeginModule, Opcode::EndModule, Opcode::ExportBinding])?;
    Ok(())
}
...
fn test_compile_import_with_alias() -> Result<()> {
    ...
    let import_node = graph.add_node(Node::Import {
        ...
        import_list: vec![
            ImportItem { name: "add".to_string(), alias: Some("plus".to_string()) },
            ImportItem { name: "sub".to_string(), alias: Some("minus".to_string()) },
        ],
        import_all: false,
    }).expect("Failed to add node");
...
fn test_compile_export_with_alias() -> Result<()> {
    ...
    let export_node = graph.add_node(Node::Export {
        export_list: vec![
            ExportItem { name: "internal_add".to_string(), alias: Some("add".to_string()) },
            ExportItem { name: "internal_sub".to_string(), alias: Some("sub".to_string()) },
        ],
    }).expect("Failed to add node");
...
fn test_compile_qualified_variable_complex() -> Result<()> {
    ...
    let math_add = graph.add_node(Node::QualifiedVariable {
    ...
    let math_call = graph.add_node(Node::Application {
        function: math_add,
        args: vec![one, two],
    }).expect("Failed to add node");
    
    let string_length = graph.add_node(Node::QualifiedVariable {
    ...
    let string_call = graph.add_node(Node::Application {
        function: string_length,
        args: vec![hello],
    }).expect("Failed to add node");
    
    let plus = graph.add_node(Node::Variable { name: "+".to_string() }).expect("Failed to add node");
    let final_app = graph.add_node(Node::Application {
        function: plus,
...
fn test_compile_all_effect_types() -> Result<()> {
    ...
    // Test IO effect
    let msg = graph.add_node(Node::Literal(Literal::String("test".to_string()))).expect("Failed to add node");
    ...
    // Test State effect
    let key = graph.add_node(Node::Literal(Literal::String("counter".to_string()))).expect("Failed to add node");
    ...
    // Test Network effect
    let url = graph.add_node(Node::Literal(Literal::String("http://example.com".to_string()))).expect("Failed to add node");
    ...
    let seq = graph.add_node(Node::List(vec![io_effect, state_effect, network_effect, random_effect, time_effect])).expect("Failed to add node");
    ...
    let effect_count = main_chunk.instructions.iter()
...
fn test_compile_async_complex() -> Result<()> {
    ...
    let await1 = graph.add_node(Node::Await { expr: fetch_call }).expect("Failed to add node");
    ...
    let await2 = graph.add_node(Node::Await { expr: process_call }).expect("Failed to add node");
    ...
    let async_node = graph.add_node(Node::Async { body: let_node }).expect("Failed to add node");
...
fn test_compile_spawn_lambda() -> Result<()> {
    ...
    let msg = graph.add_node(Node::Literal(Literal::String("worker".to_string()))).expect("Failed to add node");
    let print_call = graph.add_node(Node::Application {
        function: print_var,
        args: vec![msg],
    }).expect("Failed to add node");
...
fn test_compile_channel_complex() -> Result<()> {
    ...
    let ch2_var = graph.add_node(Node::Variable { name: "ch2".to_string() }).expect("Failed to add node");
    let recv = graph.add_node(Node::Receive { channel: ch2_var }).expect("Failed to add node");
    
    let ch1_var = graph.add_node(Node::Variable { name: "ch1".to_string() }).expect("Failed to add node");
    let send = graph.add_node(Node::Send {
        channel: ch1_var,
        value: recv,
    }).expect("Failed to add node");
...
fn test_nested_scopes() -> Result<()> {
    ...
    let a = graph.add_node(Node::Literal(Literal::Integer(1))).expect("Failed to add node");
    let _a_var = graph.add_node(Node::Variable { name: "a".to_string() }).expect("Failed to add node");
    ...
    let plus = graph.add_node(Node::Variable { name: "+".to_string() }).expect("Failed to add node");
    let inner_add = graph.add_node(Node::Application {
        function: plus,
...
fn test_module_function_exports() -> Result<()> {
    ...
    let times2 = graph.add_node(Node::Variable { name: "*".to_string() }).expect("Failed to add node");
    ...
    compile_and_check_opcodes(&graph, &[Opcode::BeginModule, Opcode::ExportBinding, Opcode::EndModule])?;
    Ok(())
}
...
fn test_module_no_exports() -> Result<()> {
    ...
    let body = graph.add_node(Node::Literal(Literal::String("private module".to_string()))).expect("Failed to add node");
    let module_node = graph.add_node(Node::Module {
...
fn test_qualified_variable_positions() -> Result<()> {
    ...
    let arg = graph.add_node(Node::Literal(Literal::Integer(5))).expect("Failed to add node");
    ...
    // In let binding
    let qual_var2 = graph.add_node(Node::QualifiedVariable {
