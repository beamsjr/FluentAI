...
fn test_compile_module_with_exports() -> Result<()> {
    ...
    let a = graph.add_node(Node::Literal(Literal::Integer(1)));
    let b = graph.add_node(Node::Literal(Literal::Integer(2)));
    let c = graph.add_node(Node::Literal(Literal::Integer(3)));
...
fn test_compile_all_effect_types() -> Result<()> {
    ...
    // Test IO effect
    let msg = graph.add_node(Node::Literal(Literal::String("test".to_string())));
    ...
    // Test Network effect
    let url = graph.add_node(Node::Literal(Literal::String("http://example.com".to_string())));
    ...
    let seq = graph.add_node(Node::List(vec![io_effect, state_effect, network_effect, random_effect, time_effect]));
...
fn test_compile_async_complex() -> Result<()> {
    ...
    let await1 = graph.add_node(Node::Await { expr: fetch_call });
    ...
    let await2 = graph.add_node(Node::Await { expr: process_call });
    ...
    let async_node = graph.add_node(Node::Async { body: let_node });
...
fn test_compile_spawn_lambda() -> Result<()> {
    ...
    let msg = graph.add_node(Node::Literal(Literal::String("worker".to_string())));
...
fn test_compile_channel_complex() -> Result<()> {
    ...
    let ch1 = graph.add_node(Node::Channel);
    let ch2 = graph.add_node(Node::Channel);
    
    let ch2_var = graph.add_node(Node::Variable { name: "ch2".to_string() });
    let recv = graph.add_node(Node::Receive { channel: ch2_var });
    
    let ch1_var = graph.add_node(Node::Variable { name: "ch1".to_string() });
...
fn test_nested_scopes() -> Result<()> {
    ...
    let a = graph.add_node(Node::Literal(Literal::Integer(1)));
    let _a_var = graph.add_node(Node::Variable { name: "a".to_string() });
...
fn test_module_function_exports() -> Result<()> {
    ...
    let times2 = graph.add_node(Node::Variable { name: "*".to_string() });
...
fn test_module_no_exports() -> Result<()> {
    ...
    let body = graph.add_node(Node::Literal(Literal::String("private module".to_string())));
    let module_node = graph.add_node(Node::Module {
...
fn test_qualified_variable_positions() -> Result<()> {
    ...
    let arg = graph.add_node(Node::Literal(Literal::Integer(5)));
