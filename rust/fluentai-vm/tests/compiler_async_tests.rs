...
fn test_compile_receive() -> Result<()> {
    ...
    let recv_node = graph
        .add_node(Node::Receive { channel: chan_var })
        .expect("Failed to add node");
...
fn test_compile_spawn() -> Result<()> {
    ...
    // Create (spawn (worker))
    let worker_var = graph
        .add_node(Node::Variable {
            name: "worker".to_string(),
        })
        .expect("Failed to add node");
    let call_worker = graph
        .add_node(Node::Application {
            function: worker_var,
            args: vec![],
        })
    ...
    let spawn_node = graph
        .add_node(Node::Spawn { expr: call_worker })
        .expect("Failed to add node");
...
fn test_compile_channel_operations_sequence() -> Result<()> {
    ...
    let seq = graph
        .add_node(Node::List(vec![send_node, recv_node]))
        .expect("Failed to add node");
...
fn test_compile_spawn_multiple() -> Result<()> {
    ...
    let call1 = graph
        ...
        .expect("Failed to add node");
    let spawn1 = graph
        .add_node(Node::Spawn { expr: call1 })
        .expect("Failed to add node");

    let worker_var2 = graph
        .add_node(Node::Variable {
            name: "worker".to_string(),
        })
        .expect("Failed to add node");
    ...
    let call2 = graph
        ...
        .expect("Failed to add node");
    let spawn2 = graph
        .add_node(Node::Spawn { expr: call2 })
        .expect("Failed to add node");

    let list_node = graph
        .add_node(Node::List(vec![spawn1, spawn2]))
        .expect("Failed to add node");
...
fn test_compile_channel_in_closure() -> Result<()> {
    ...
    let seq = graph
        .add_node(Node::List(vec![send_node, ch_var2]))
        .expect("Failed to add node");
