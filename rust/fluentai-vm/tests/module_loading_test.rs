use fluentai_vm::{VM, VMBuilder, Compiler, CompilerOptions, OptimizationLevel, Value, VMError};
use fluentai_core::ast::{Graph, Node, Literal, ImportItem};
...
fn test_module_loading_and_execution() {
    ...
    let math_file = create_test_module(temp_path, "math", r#"
nil
"#);
    ...
    let double_var = graph.add_node(Node::Variable { name: "double".to_string() }).unwrap();
    let call_double = graph.add_node(Node::Application {
        function: double_var,
        args: vec![five],
    }).unwrap();
    ...
    let import_node = graph.add_node(Node::Import {
        ...
        import_list: vec![
            ImportItem { name: "double".to_string(), alias: None },
            ImportItem { name: "square".to_string(), alias: None },
        ],
        import_all: false,
    }).unwrap();
...
fn test_module_import_all() {
    ...
    create_test_module(temp_path, "utils", r#"
...
"#);
    ...
    let add1_var = graph.add_node(Node::Variable { name: "add1".to_string() }).unwrap();
    let call_add1 = graph.add_node(Node::Application {
        function: add1_var,
        args: vec![ten],
    }).unwrap();
