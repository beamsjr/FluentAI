...
use anyhow::Result;
use fluentai_core::ast::{ExportItem, Graph, ImportItem, Literal, Node};
...
fn test_compile_import_all() -> Result<()> {
    ...
    assert!(main_chunk
        .instructions
        .iter()
        .any(|instr| instr.opcode == Opcode::ImportAll));
...
fn test_compile_qualified_variable() -> Result<()> {
    ...
    assert!(main_chunk
        .instructions
        .iter()
        .any(|instr| instr.opcode == Opcode::LoadQualified));
...
fn test_compile_module_with_nested_imports() -> Result<()> {
    ...
    // Create a sequence
    let seq_body = graph
        .add_node(Node::List(vec![import_node, app]))
        .expect("Failed to add node");
...
fn test_compile_import_multiple_modules() -> Result<()> {
    ...
    let seq = graph
        .add_node(Node::List(vec![import1, import2, add_app]))
        .expect("Failed to add node");
