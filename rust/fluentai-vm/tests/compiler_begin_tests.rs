...
use fluentai_vm::{
    compiler::Compiler,
    bytecode::{Opcode, Instruction, Chunk},
};
...
fn test_compile_begin_expressions_with_side_effects() {
    ...
    let pop_count = opcodes.iter().filter(|op| matches!(op, Opcode::Pop)).count();
...
fn test_begin_error_recovery() {
    ...
    let begin_node = Node::Begin { exprs: vec![999.into(), 1000.into()] };
