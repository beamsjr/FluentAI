...
fn test_import_all_not_implemented() -> Result<()> {
    ...
    // Import-all is now implemented, so compilation should succeed
    let options = CompilerOptions {
    ...
    let has_import_all = bytecode.chunks[0]
        .instructions
        .iter()
        .any(|i| i.opcode == Opcode::ImportAll);
...
fn test_module_with_complex_exports() -> Result<()> {
    ...
    let double_body = graph
        .add_node(Node::List(vec![x_var1, x_var2]))
        .expect("Failed to add node");
    let double_fn = graph
    ...
    let body = graph
        .add_node(Node::List(vec![import_node, let_node]))
        .expect("Failed to add node");
...
fn test_export_with_complex_aliases() -> Result<()> {
    ...
    let seq = graph
        .add_node(Node::List(vec![let_node, export_node]))
        .expect("Failed to add node");
...
fn test_async_with_error_handling() -> Result<()> {
    ...
    let await_node = graph
        .add_node(Node::Await { expr: risky_call })
        .expect("Failed to add node");
    ...
    let async_node = graph
        .add_node(Node::Async { body: if_node })
        .expect("Failed to add node");
...
fn test_nested_async_operations() -> Result<()> {
    ...
    let fetch_call = graph
        ...
        .expect("Failed to add node");
    let inner_await = graph
        .add_node(Node::Await { expr: fetch_call })
        .expect("Failed to add node");
    let inner_async = graph
        .add_node(Node::Async { body: inner_await })
        .expect("Failed to add node");
...
fn test_spawn_with_captured_variables() -> Result<()> {
    ...
    assert!(
        bytecode.chunks.len() >= 2,
        "Should have main and lambda chunks"
    );
...
fn test_complex_await_chains() -> Result<()> {
    ...
    let fetch = graph
        ...
        .expect("Failed to add node");
    let url1 = graph
        .add_node(Node::Literal(Literal::String("url1".to_string())))
        .expect("Failed to add node");
    let fetch_call = graph
        .add_node(Node::Application {
            function: fetch,
...
fn test_async_in_conditional_branches() -> Result<()> {
    ...
    let fast_call = graph
        ...
        .expect("Failed to add node");
    let fast_await = graph
        .add_node(Node::Await { expr: fast_call })
        .expect("Failed to add node");
    let then_async = graph
        .add_node(Node::Async { body: fast_await })
        .expect("Failed to add node");
    ...
    let slow_call = graph
        ...
        .expect("Failed to add node");
    let slow_await = graph
        .add_node(Node::Await { expr: slow_call })
        .expect("Failed to add node");
    let else_async = graph
        .add_node(Node::Async { body: slow_await })
        .expect("Failed to add node");
...
fn test_constructor_many_args() -> Result<()> {
    ...
    let record_ctor = graph
        .add_node(Node::Variable {
            name: "Record".to_string(),
        })
        .expect("Failed to add node");

    let args: Vec<NodeId> = (1..=5)
        .flat_map(|i| {
            vec![
                graph
                    .add_node(Node::Literal(Literal::String(format!("field{}", i))))
                    .expect("Failed to add node"),
                graph
...
fn test_constructor_as_first_class_value() -> Result<()> {
    ...
    let make_nil_var = graph
        .add_node(Node::Variable {
            name: "make-nil".to_string(),
        })
        .expect("Failed to add node");
    let call_make_nil = graph
        .add_node(Node::Application {
...
fn test_gc_let_multiple_bindings() -> Result<()> {
    ...
    let n5 = graph
        ...
        .expect("Failed to add node");
    let n6 = graph
        .add_node(Node::Literal(Literal::Integer(6)))
        .expect("Failed to add node");
    let list2 = graph
        .add_node(Node::List(vec![n4, n5, n6]))
        .expect("Failed to add node");

    let n7 = graph
        .add_node(Node::Literal(Literal::Integer(7)))
        .expect("Failed to add node");
    let n8 = graph
        .add_node(Node::Literal(Literal::Integer(8)))
        .expect("Failed to add node");
    let n9 = graph
        .add_node(Node::Literal(Literal::Integer(9)))
        .expect("Failed to add node");
    let list3 = graph
        .add_node(Node::List(vec![n7, n8, n9]))
        .expect("Failed to add node");
    ...
    let binding1 = graph
        .add_node(Node::List(vec![a_sym, list1]))
        .expect("Failed to add node");
    let binding2 = graph
        .add_node(Node::List(vec![b_sym, list2]))
        .expect("Failed to add node");
    let binding3 = graph
        .add_node(Node::List(vec![c_sym, list3]))
        .expect("Failed to add node");

    let bindings = graph
        .add_node(Node::List(vec![binding1, binding2, binding3]))
        .expect("Failed to add node");
    ...
    let c_var = graph
        ...
        .expect("Failed to add node");
    let body = graph
        .add_node(Node::List(vec![a_var, b_var, c_var]))
        .expect("Failed to add node");
...
fn test_gc_let_with_complex_expressions() -> Result<()> {
    ...
    let data_sym = graph
        ...
        .expect("Failed to add node");
    let binding1 = graph
        .add_node(Node::List(vec![fn_sym, square_fn]))
        .expect("Failed to add node");
    let binding2 = graph
        .add_node(Node::List(vec![data_sym, map_app]))
        .expect("Failed to add node");
    let bindings = graph
        .add_node(Node::List(vec![binding1, binding2]))
        .expect("Failed to add node");

    // Body
    let reduce_var = graph
        .add_node(Node::Variable {
...
fn test_gc_let_error_no_body() -> Result<()> {
    ...
    let ten = graph
        ...
        .expect("Failed to add node");
    let binding = graph
        .add_node(Node::List(vec![x_sym, ten]))
        .expect("Failed to add node");
    let bindings = graph
        .add_node(Node::List(vec![binding]))
        .expect("Failed to add node");
...
fn test_nested_gc_let() -> Result<()> {
    ...
    let inner_sym = graph
        ...
        .expect("Failed to add node");
    let inner_binding = graph
        .add_node(Node::List(vec![inner_sym, cons_app]))
        .expect("Failed to add node");
    let inner_bindings = graph
        .add_node(Node::List(vec![inner_binding]))
        .expect("Failed to add node");
    let inner_var = graph
        .add_node(Node::Variable {
            name: "inner".to_string(),
    ...
    let outer_sym = graph
        ...
        .expect("Failed to add node");
    let outer_binding = graph
        .add_node(Node::List(vec![outer_sym, outer_list]))
        .expect("Failed to add node");
    let outer_bindings = graph
        .add_node(Node::List(vec![outer_binding]))
        .expect("Failed to add node");

    let outer_gc_let = graph
        .add_node(Node::Application {
            function: gc_let,
