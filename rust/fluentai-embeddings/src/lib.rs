...
use fluentai_core::ast::{ContextMemory, EmbeddingId, Graph, Node, NodeId};
use fluentai_optimizer::ml_hints::{MLOptimizationHints, ProgramFeatures};
...
impl EmbeddingService {
    ...
    pub fn new(generator: Box<dyn EmbeddingGenerator>, storage: Box<dyn EmbeddingStorage>) -> Self {
        Self {
            generator,
            storage,
    ...
    pub async fn generate_embedding(&self, graph: &Graph, node_id: NodeId) -> Result<EmbeddingId> {
    ...
    fn extract_context(&self, graph: &Graph, node_id: NodeId) -> Result<EmbeddingContext> {
        ...
        let documentation = metadata.as_ref().and_then(|m| m.documentation_id.clone());
