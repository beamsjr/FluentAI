...
use crate::{EmbeddingId, EmbeddingService};
use anyhow::Result;
use fluentai_core::documentation::{
    BuiltinDoc, Documentation, DocumentationRegistry, KeywordDoc, OperatorDoc,
};
...
impl DocumentationEmbeddingService {
    ...
    async fn generate_doc_embedding(
        &mut self,
        name: &str,
        doc: &Documentation,
    ) -> Result<EmbeddingId> {
    ...
    fn extract_keyword_features(&self, kw: &KeywordDoc) -> HashMap<String, f32> {
        ...
        features.insert(
            "kw_control_flow".to_string(),
            if kw.keyword == "if" || kw.keyword == "cond" || kw.keyword == "match" {
                1.0
            } else {
                0.0
            },
        ...
        features.insert(
            "kw_binding".to_string(),
            if kw.keyword == "let" || kw.keyword == "letrec" || kw.keyword == "define" {
                1.0
            } else {
                0.0
            },
        ...
        features.insert(
            "kw_function".to_string(),
            if kw.keyword == "lambda" || kw.keyword == "fn" {
                1.0
            } else {
                0.0
            },
    ...
    fn add_text_features(&self, features: &mut HashMap<String, f32>, prefix: &str, text: &str) {
        ...
        let keywords = [
            "function", "return", "value", "list", "string", "number", "error", "effect", "async",
            "pure", "side", "mutation",
        ];
    ...
    fn add_category_enum_features(
        &self,
        features: &mut HashMap<String, f32>,
        category: &fluentai_core::documentation::DocumentationCategory,
    ) {
    ...
    pub async fn search_by_similarity(
        ...
        let query_id = self
            .embedding_service
            .store_embedding(query_embedding)
            .await?;

        // Find similar embeddings
        let similar = self
            .embedding_service
            .find_similar(query_id, limit * 2, threshold)
            .await?;
