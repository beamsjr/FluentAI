...
impl FeatureBasedGenerator {
    ...
    pub fn generate_from_features(&self, features: &std::collections::HashMap<String, f32>) -> Result<Vec<f32>> {
...
impl EmbeddingGenerator for FeatureBasedGenerator {
    async fn generate(&self, context: &EmbeddingContext) -> Result<Vec<f32>> {
        ...
        if let Some(context_mem) = &context.context_memory {
            ...
            // Performance hints
            for hint_type in &[
                "ShouldInline", "ShouldUnroll", "CanVectorize", 
                "CanParallelize", "ShouldMemoize"
            ] {
                let has_hint = context_mem.performance_hints.iter()
