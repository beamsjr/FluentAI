...
use fluentai_embeddings::{
    ...
    similarity::{cosine_similarity, kmeans_cluster, SimilarityMetric, knn_search},
    doc_embeddings::DocumentationEmbeddingService,
};
use fluentai_core::ast::{Graph, Node, Literal, NodeMetadata, ContextMemory, UsageStatistics, EmbeddingId, NodeId};
...
async fn test_full_embedding_workflow() {
    ...
    let first_embedding_id = graph.get_context_memory(first_node_id)
        .unwrap()
        .embedding_id
        .unwrap();
    
    let similar = service.find_similar(first_embedding_id, 5, 0.5).await.unwrap();
...
async fn test_clustering_workflow() {
    ...
    let integers_in_first_cluster = assignments[0..5].iter().filter(|&&c| c == first_cluster).count();
    
    // At least 3 out of 5 integers should be in the same cluster
    assert!(integers_in_first_cluster >= 3, "Integers should tend to cluster together");
}
...
async fn test_documentation_integration() {
    ...
    assert!(!doc_embeddings.is_empty(), "Should have generated documentation embeddings");
    
    // Check that lambda keyword embedding was generated
    assert!(doc_embeddings.contains_key("kw_lambda"), "Should have lambda keyword embedding");
...
async fn test_performance_hints_impact() {
    ...
    assert!(non_zero2 > 0, "Second embedding should have non-zero values");
    ...
    if norm1 > 0.0 && norm2 > 0.0 {
        assert!((norm1 - 1.0).abs() < 1e-6, "First embedding not normalized: {}", norm1);
        assert!((norm2 - 1.0).abs() < 1e-6, "Second embedding not normalized: {}", norm2);
        
        // Only check similarity if both embeddings are non-zero
        let similarity = cosine_similarity(&emb1, &emb2);
        // Just verify it's a valid similarity value
        assert!(similarity >= -1.0 && similarity <= 1.0, "Invalid similarity: {}", similarity);
    }
}
...
async fn test_similarity_search_workflow() {
    ...
    let test_nodes = vec![
        ...
        Node::Variable { name: "y".to_string() },
    ];
...
async fn test_metadata_enriched_embeddings() {
    ...
    context.rationale = Some("This is an important constant used throughout the system".to_string());
    context.semantic_tags = vec!["constant".to_string(), "important".to_string(), "configuration".to_string()];
    ...
    assert!(non_zero_count > 0, "Expected at least some non-zero values, got {}", non_zero_count); // Should have some features
...
fn create_complex_graph() -> Graph {
    ...
    let lambda_body = Node::Variable { name: "x".to_string() };
    ...
    graph.set_metadata(int_id, NodeMetadata {
        span: Some((1, 10)),
        type_info: Some("Integer".to_string()),
        is_pure: Some(true),
        annotations: vec![],
