...
async fn test_find_similar() {
    ...
    assert!(
        (self_match.unwrap().1 - 1.0).abs() < 1e-6,
        "Self-match should have similarity 1.0"
    );
...
async fn test_store_embedding_directly() {
    ...
    let id = service
        .store_embedding(custom_embedding.clone())
        .await
        .unwrap();
...
async fn test_service_with_file_storage() {
    ...
    assert!(temp_dir
        .path()
        .join(format!("{}.emb", embedding_id.0))
        .exists());
}
