...
use fluentai_core::ast::{Node, Literal, Graph, ContextMemory, NodeMetadata, UsageStatistics};
...
async fn test_node_features_all_types() {
    ...
    let test_cases = vec![
        ...
        (Node::Literal(Literal::String("test".to_string())), "String literal"),
        (Node::Literal(Literal::Boolean(true)), "Boolean literal"),
        (Node::Literal(Literal::Nil), "Nil literal"),
        (Node::Variable { name: "x".to_string() }, "Variable"),
        (Node::List(vec![]), "List"),
    ...
    for (node, description) in test_cases {
        ...
        let norm: f32 = embedding.iter().map(|x| x * x).sum::<f32>().sqrt();
        assert!((norm - 1.0).abs() < 1e-6, "Not normalized for {}", description);
    }
}
...
async fn test_generate_with_full_context() {
    ...
    assert!(non_zero_count >= 40, "Expected at least 40 non-zero values, got {}", non_zero_count);
}
...
async fn test_generate_different_dimensions() {
    for dim in [64, 128, 256, 512, 1024] {
        ...
        // Should be normalized
        let norm: f32 = embedding.iter().map(|x| x * x).sum::<f32>().sqrt();
        assert!((norm - 1.0).abs() < 1e-6, "Not normalized for dimension {}", dim);
    }
}
