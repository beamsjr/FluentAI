...
async fn test_embeddings_are_unique() {
    ...
    assert_eq!(
        ids.len(),
        unique_ids.len(),
        "All embedding IDs should be unique"
    );
}
...
async fn test_consistent_embeddings() {
    ...
    assert_eq!(
        keys1, keys2,
        "Should generate embeddings for the same documentation"
    );
}
...
async fn test_concurrent_service_creation() {
    ...
    let tasks: Vec<_> = (0..5)
        .map(|_| async { DocumentationEmbeddingService::new().await })
        .collect();
...
async fn test_search_by_similarity() {
    ...
    let results = service
        .search_by_similarity("math function", 10, 0.5)
        .await
        .unwrap();
