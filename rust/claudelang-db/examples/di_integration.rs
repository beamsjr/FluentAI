...
use claudelang_effects::{EffectHandlerProvider, EffectHandlerBuilder};
use anyhow::Result;

#[tokio::main]
async fn main() -> Result<()> {
    println!("=== ClaudeLang Database DI Integration Example ===\n");
...
async fn method2_services_builder() -> Result<()> {
    ...
    // Resolve connection
    let conn = container.resolve::<Arc<claudelang_db::DbConnection>>()?;
    ...
    let _result = with_transaction(conn.clone(), |tx| {
        Box::pin(async move {
            ...
            tx.execute(
                ...
                vec![
                    claudelang_vm::bytecode::Value::String("Widget".to_string()),
                    claudelang_vm::bytecode::Value::Float(9.99),
                    claudelang_vm::bytecode::Value::String("Gadget".to_string()),
                    claudelang_vm::bytecode::Value::Float(19.99),
                ]
            ).await?;
...
impl ApplicationService {
    ...
    async fn run(&self) -> Result<()> {
        ...
        if let Ok(effect_provider) = self.container.resolve::<Arc<EffectHandlerProvider>>() {
            ...
            use claudelang_effects::EffectType;
    ...
    async fn use_database(&self, conn: Arc<claudelang_db::DbConnection>) -> Result<()> {
        // Create table first
        conn.execute_raw_unsafe(
        ...
        conn.execute(
            ...
            vec![
                claudelang_vm::bytecode::Value::Int(1),
                claudelang_vm::bytecode::Value::String("dark".to_string()),
                claudelang_vm::bytecode::Value::String("en".to_string()),
                claudelang_vm::bytecode::Value::Bool(true),
                claudelang_vm::bytecode::Value::String("2024-01-01".to_string()),
            ]
        ).await?;
        ...
        let rows = conn.fetch_all(
            ...
            vec![claudelang_vm::bytecode::Value::Int(1)]
        ).await?;
