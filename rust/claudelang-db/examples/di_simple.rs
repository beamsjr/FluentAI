...
fn method1_extension_trait() -> Result<()> {
    ...
    let pool_result = container.resolve::<Arc<claudelang_db::ConnectionPool>>();
    match pool_result {
...
impl DatabaseServiceProvider for MockDatabaseProvider {
    fn get_connection(&self) -> claudelang_db::error::DbResult<Arc<claudelang_db::DbConnection>> {
        Err(claudelang_db::error::DbError::Connection("Mock provider - no real connection".into()))
    }
    
    fn get_transaction_manager(&self) -> claudelang_db::error::DbResult<Arc<claudelang_db::TransactionManager>> {
        Err(claudelang_db::error::DbError::Connection("Mock provider - no real transaction manager".into()))
    }
    
    fn get_migration_runner(&self) -> claudelang_db::error::DbResult<Arc<claudelang_db::MigrationRunner>> {
        Err(claudelang_db::error::DbError::Connection("Mock provider - no real migration runner".into()))
    }
}
