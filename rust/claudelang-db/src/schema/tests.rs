...
mod tests {
    ...
    fn test_field_type_to_sql() {
        ...
        assert_eq!(FieldType::String(Some(50)).to_sql(SqlDialect::Postgres), "VARCHAR(50)");
        assert_eq!(FieldType::String(None).to_sql(SqlDialect::Postgres), "VARCHAR");
        
        // Test decimal
        assert_eq!(FieldType::Decimal(10, 2).to_sql(SqlDialect::MySQL), "DECIMAL(10, 2)");
    ...
    fn test_field_builder() {
        ...
        assert!(field_with_default.constraints.iter().any(|c| 
            matches!(c, Constraint::Default(val) if val == "CURRENT_TIMESTAMP")
        ));
    }
    ...
    fn test_schema_builder() {
        let schema = SchemaBuilder::new("users")
            ...
            .add_field_with("email", FieldType::String(Some(255)), |f| f.unique().not_null())
        ...
        assert!(schema.fields[1].constraints.iter().any(|c| matches!(c, Constraint::Unique)));
        assert!(schema.fields[2].constraints.iter().any(|c| 
            matches!(c, Constraint::Check(expr) if expr == "age >= 0")
        ));
    }
        ...
        let schema = SchemaBuilder::new("orders")
            ...
            .add_field(
                Field::new("user_id", FieldType::Int)
                    .not_null()
                    .foreign_key("users", "id", ForeignKeyAction::Restrict, ForeignKeyAction::Cascade)
            )
    ...
    fn test_complex_schema() {
        let schema = SchemaBuilder::new("user_profiles")
            ...
            .add_field_with("verified", FieldType::Bool, |f| f.not_null().default("false"))
    ...
    fn test_field_nullable_default() {
        ...
        let required_field = Field::new("required_field", FieldType::String(None))
            .not_null();
    ...
    fn test_binary_field_types() {
        assert_eq!(FieldType::Binary(Some(16)).to_sql(SqlDialect::MySQL), "BINARY(16)");
        assert_eq!(FieldType::Binary(None).to_sql(SqlDialect::Postgres), "BYTEA");
    ...
    fn test_datetime_field_types() {
        ...
        assert_eq!(FieldType::Timestamp.to_sql(SqlDialect::Postgres), "TIMESTAMP");
    }
}