//! Database migration system for ClaudeLang
...
impl MigrationRepository {
    ...
    pub async fn table_exists(&self) -> DbResult<bool> {
        ...
        let result = self.connection.fetch_all(
            ...
            vec![claudelang_vm::bytecode::Value::String(self.table_name.clone())]
        ).await;
    ...
    pub async fn is_applied(&self, version: &str) -> DbResult<bool> {
        ...
        let rows = self.connection.fetch_all(
            &query,
            vec![claudelang_vm::bytecode::Value::String(version.to_string())]
        ).await?;
    ...
    pub async fn record_migration(
        ...
        let params = vec![
            claudelang_vm::bytecode::Value::String(metadata.version.clone()),
            claudelang_vm::bytecode::Value::String(metadata.name.clone()),
            metadata.description.as_ref()
                .map(|d| claudelang_vm::bytecode::Value::String(d.clone()))
                .unwrap_or(claudelang_vm::bytecode::Value::Nil),
            claudelang_vm::bytecode::Value::String(metadata.checksum.clone()),
            claudelang_vm::bytecode::Value::String(
                metadata.applied_at.unwrap_or_else(Utc::now).to_rfc3339()
            ),
            metadata.execution_time_ms
                .map(|ms| claudelang_vm::bytecode::Value::Int(ms as i64))
                .unwrap_or(claudelang_vm::bytecode::Value::Nil),
        ];
