...
mod transaction_tests {
    ...
    fn test_retryable_errors() {
        assert!(is_retryable_error(&DbError::Query("deadlock detected".into())));
        assert!(is_retryable_error(&DbError::Transaction("serialization failure".into())));
        assert!(is_retryable_error(&DbError::Query("could not serialize access".into())));
        assert!(!is_retryable_error(&DbError::Connection("connection refused".into())));
    }
}
