...
use crate::{Manifest, PackageError, Result, Version, VersionReq};
...
impl Registry for LocalRegistry {
    ...
    fn publish(&self, manifest: &Manifest, tarball: &Path, _token: &str) -> Result<()> {
        ...
        package.latest = package.versions.iter().map(|v| &v.version).max().cloned();
    ...
    fn get_matching_versions(&self, name: &str, req: &VersionReq) -> Result<Vec<Version>> {
        ...
        let mut matching: Vec<_> = package
            .versions
            .into_iter()
            .map(|v| v.version)
