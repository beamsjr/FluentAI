...
use crate::{PackageError, Result, Version, VersionReq, Registry, Manifest, manifest::Dependency};
...
impl DependencyResolver {
    ...
    pub fn resolve(&mut self, manifest: &Manifest, include_dev: bool) -> Result<Vec<ResolvedDependency>> {
    ...
    fn find_best_version(&mut self, name: &str, req: &VersionReq) -> Result<Version> {
        ...
        let versions = if let Some(cached) = self.cache.get(name) {
            ...
            let all_versions = self.registry.get_matching_versions(name, &VersionReq::any())?;
