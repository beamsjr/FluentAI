...
use crate::{
    lockfile::LockedPackage, resolver::ResolvedDependency, DependencyResolver, Lockfile, Manifest,
    PackageConfig, PackageError, Registry, Result, Version,
};
use std::fs;
...
impl PackageInstaller {
    ...
    fn install_package(&self, dep: &ResolvedDependency, lockfile: &mut Lockfile) -> Result<()> {
        ...
        let tarball_path =
            self.registry
                .download(&dep.name, &dep.version, &self.config.temp_dir)?;
    ...
    fn print_tree_node(
        ...
        println!(
            "{}{}{}@{}",
            prefix, connector, package.name, package.version
        );
...
mod tests {
    ...
    fn test_installer_creation() {
        ...
        let installer = PackageInstaller::new(config, registry, temp_dir.path().to_path_buf());
