//! Benchmarks for FluentAi standard library functions
...
fn benchmark_list_operations(c: &mut Criterion) {
    ...
    c.bench_function("list_append_small", |b| {
        ...
        b.iter(|| {
            append
                .call(&[black_box(list.clone()), black_box(Value::Integer(4))])
                .unwrap()
        });
    });
    ...
    c.bench_function("list_reverse_100", |b| {
        ...
        b.iter(|| reverse.call(&[black_box(list.clone())]).unwrap());
    });
    ...
    c.bench_function("list_length_1000", |b| {
        ...
        b.iter(|| length.call(&[black_box(list.clone())]).unwrap());
    });

    c.bench_function("range_1000", |b| {
        let range = stdlib.get("range").unwrap();
        b.iter(|| range.call(&[black_box(Value::Integer(1000))]).unwrap());
    });
}
...
fn benchmark_string_operations(c: &mut Criterion) {
    ...
    c.bench_function("string_concat_10", |b| {
        ...
        let strings: Vec<Value> = (0..10)
            ...
            .collect();
        b.iter(|| concat.call(&black_box(strings.clone())).unwrap());
    });
    ...
    c.bench_function("string_split_csv", |b| {
        ...
        b.iter(|| {
            split
                .call(&[black_box(csv.clone()), black_box(delim.clone())])
                .unwrap()
        });
    });
    ...
    c.bench_function("string_upcase_long", |b| {
        ...
        b.iter(|| upcase.call(&[black_box(text.clone())]).unwrap());
    });
}
...
fn benchmark_math_operations(c: &mut Criterion) {
    ...
    c.bench_function("math_add_100_ints", |b| {
        ...
        b.iter(|| add.call(&black_box(numbers.clone())).unwrap());
    });

    c.bench_function("math_sqrt_float", |b| {
        let sqrt = stdlib.get("sqrt").unwrap();
        b.iter(|| sqrt.call(&[black_box(Value::Float(16384.0))]).unwrap());
    });
...
fn benchmark_collection_operations(c: &mut Criterion) {
    ...
    c.bench_function("list_flatten_nested", |b| {
        ...
        let nested = Value::List(
            (0..50)
                .map(|i| Value::List(vec![Value::Integer(i * 2), Value::Integer(i * 2 + 1)]))
                .collect(),
        );
        b.iter(|| flatten.call(&[black_box(nested.clone())]).unwrap());
    });
}
