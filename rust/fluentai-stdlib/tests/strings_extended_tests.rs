...
fn test_string_pad_functions() {
    ...
    let result = pad_center
        .call(&[Value::String("center".to_string()), Value::Integer(10)])
        .unwrap();
...
fn test_string_repeat() {
    ...
    let result = repeat
        .call(&[Value::String("na".to_string()), Value::Integer(4)])
        .unwrap();
...
fn test_string_reverse() {
    ...
    // Test with Unicode
    let result = reverse
        .call(&[Value::String("café ☕".to_string())])
        .unwrap();
...
fn test_string_format() {
    ...
    assert_eq!(
        result,
        Value::String("Hello Alice, welcome to FluentAI!".to_string())
    );
    ...
    assert_eq!(
        result,
        Value::String("\"Bob\" is 25 years old and has 123.45 dollars".to_string())
    );

    // Special characters
    let result = format
        .call(&[Value::String(
            "Line 1~%Line 2~%~%100~~ complete".to_string(),
        )])
        .unwrap();
    assert_eq!(
        result,
    ...
    assert_eq!(
        result,
        Value::String("List: [1 2 3], Map: {key: value}".to_string())
    );
}
