...
use fluentai_stdlib::io_effects::{self, IOEffectContext, LoggingIOHandler, set_io_context};
use std::sync::Arc;
...
fn test_print_line_with_effects() {
    ...
    let result = io_effects::print_line_with_effects(&[
...
fn test_file_operations_with_logging_handler() {
    ...
    assert!(result.unwrap_err().to_string().contains("File operations not allowed in sandbox"));
...
fn test_read_line_with_effects() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Input not allowed in sandbox"));
...
fn test_io_disabled() {
    ...
    assert!(err_msg.contains("I/O operations are not allowed") || err_msg.contains("not in allowed paths"),
            "Expected 'I/O operations are not allowed' or 'not in allowed paths', got: {}", err_msg);
...
fn test_read_only_mode() {
    ...
    assert!(err_msg.contains("read-only mode") || err_msg.contains("File operations not allowed in sandbox"),
            "Expected read-only or sandbox error, got: {}", err_msg);
...
fn test_path_restrictions() {
    ...
    let result = io_effects::file_read_with_effects(&[Value::String("/allowed/test.txt".to_string())]);
    assert!(result.is_err());
    
    // Disallowed path should fail (LoggingIOHandler returns its own error)
    let result = io_effects::file_read_with_effects(&[Value::String("/not-allowed/test.txt".to_string())]);
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    // The path check happens before the handler is called
    assert!(err_msg.contains("not in allowed paths") || err_msg.contains("File operations not allowed in sandbox"),
            "Expected path error or sandbox error, got: {}", err_msg);
