...
pub fn register(registry: &mut StdlibRegistry) {
    registry.register_all(vec![
        // Padding functions
        StdlibFunction::pure("string-pad-left", string_pad_left, 2, Some(3), "Pad string on the left"),
        StdlibFunction::pure("string-pad-right", string_pad_right, 2, Some(3), "Pad string on the right"),
        StdlibFunction::pure("string-pad-center", string_pad_center, 2, Some(3), "Pad string on both sides"),
        
        // Other string functions
        StdlibFunction::pure("string-repeat", string_repeat, 2, Some(2), "Repeat string n times"),
        StdlibFunction::pure("string-reverse", string_reverse, 1, Some(1), "Reverse a string"),
        StdlibFunction::pure("string-index-of", string_index_of, 2, Some(3), "Find index of substring"),
        StdlibFunction::pure("string-format", string_format, 1, None, "Format string with arguments"),
    ]);
}
        ...
        if ch == '~' {
            if let Some(&next_ch) = chars.peek() {
                ...
                match next_ch {
                    ...
                    _ => {
                        return Err(anyhow!("string-format: unknown format directive ~{}", next_ch));
                    }
                }
            } else {
...
mod tests {
    ...
    fn test_string_format() {
        ...
        let result = string_format(&[
            Value::String("Line 1~%Line 2".to_string())
        ]).unwrap();
