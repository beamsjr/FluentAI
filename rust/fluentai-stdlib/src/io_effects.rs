...
{
    IO_CONTEXT.with(|ctx| f(&*ctx.borrow()))
}
    ...
    with_io_context(|ctx| {
        ...
        if let Some(allowed) = &ctx.allowed_paths {
            ...
            let allowed_match = allowed
                .iter()
                .any(|allowed_path| path_to_check.starts_with(allowed_path));
...
pub fn file_read_with_effects(args: &[Value]) -> Result<Value> {
    ...
    with_io_context(|ctx| {
        if let Some(handler) = &ctx.io_handler {
            ...
            let content = std::fs::read_to_string(path).map_err(|e| anyhow!("file-read: {}", e))?;
            Ok(Value::String(content))
        }
    })
}
...
pub fn file_write_with_effects(args: &[Value]) -> Result<Value> {
    ...
    with_io_context(|ctx| {
        if let Some(handler) = &ctx.io_handler {
            ...
            // Default implementation
            std::fs::write(path, content).map_err(|e| anyhow!("file-write: {}", e))?;
        }
        Ok(Value::Nil)
    })
}
...
pub fn file_delete_with_effects(args: &[Value]) -> Result<Value> {
    ...
    with_io_context(|ctx| {
        if let Some(handler) = &ctx.io_handler {
            ...
            // Default implementation
            std::fs::remove_file(path).map_err(|e| anyhow!("file-delete: {}", e))?;
        }
        Ok(Value::Nil)
    })
}
...
pub fn dir_list_with_effects(args: &[Value]) -> Result<Value> {
    ...
    with_io_context(|ctx| {
        if let Some(handler) = &ctx.io_handler {
            ...
            // Default implementation
            let entries = std::fs::read_dir(path).map_err(|e| anyhow!("dir-list: {}", e))?;
...
pub fn dir_create_with_effects(args: &[Value]) -> Result<Value> {
    ...
    with_io_context(|ctx| {
        if let Some(handler) = &ctx.io_handler {
            ...
            // Default implementation
            std::fs::create_dir_all(path).map_err(|e| anyhow!("dir-create: {}", e))?;
        }
        Ok(Value::Nil)
    })
}
...
pub fn current_directory_with_effects(_args: &[Value]) -> Result<Value> {
    with_io_context(|ctx| {
        ...
        if let Some(handler) = &ctx.io_handler {
            ...
            // Default implementation
            let dir = std::env::current_dir().map_err(|e| anyhow!("current-directory: {}", e))?;
