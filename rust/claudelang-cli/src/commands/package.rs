...
use claudelang_package::{Manifest, PackageInstaller, Registry};
use claudelang_package::registry::{LocalRegistry, HttpRegistry};
use std::sync::Arc;

pub async fn handle_package_command(cmd: PackageCommands, config: &Config) -> Result<()> {
    // Create package config from CLI config
    let mut pkg_config = claudelang_package::PackageConfig::load()
        .unwrap_or_default();
...
fn add_dependency(
    ...
    use claudelang_package::manifest::Dependency;
