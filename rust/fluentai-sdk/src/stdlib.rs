...
use fluentai_core_lib::{HostFunction, RuntimeEngine};
...
pub fn register_core(engine: &RuntimeEngine) -> Result<()> {
    let functions = vec![
        ...
        // Equality
        HostFunction::new("equal?", 2, |args| Ok(Value::Boolean(args[0] == args[1]))),
        // Not
...
pub fn register_math(engine: &RuntimeEngine) -> Result<()> {
    let functions = vec![
        ...
        HostFunction::new("abs", 1, |args| match &args[0] {
        ...
        HostFunction::new("sqrt", 1, |args| match &args[0] {
        ...
        HostFunction::new("pow", 2, |args| match (&args[0], &args[1]) {
        ...
        HostFunction::new("ceil", 1, |args| match &args[0] {
        ...
        HostFunction::new("round", 1, |args| match &args[0] {
...
pub fn register_string(engine: &RuntimeEngine) -> Result<()> {
    let functions = vec![
        HostFunction::new("string-length", 1, |args| match &args[0] {
        ...
        HostFunction::new("string-append", 0, |args| {
            ...
            for arg in args {
                match arg {
                    ...
                    _ => {
                        return Err(fluentai_core_lib::RuntimeError::host(
        ...
        .variadic(),
        HostFunction::new("string-upcase", 1, |args| match &args[0] {
            Value::String(s) => Ok(Value::String(s.to_uppercase())),
            _ => Err(fluentai_core_lib::RuntimeError::host(
                "string-upcase expects a string",
            )),
        }),
        HostFunction::new("string-downcase", 1, |args| match &args[0] {
            Value::String(s) => Ok(Value::String(s.to_lowercase())),
            _ => Err(fluentai_core_lib::RuntimeError::host(
                "string-downcase expects a string",
            )),
        }),
        HostFunction::new("string->number", 1, |args| match &args[0] {
            Value::String(s) => match s.parse::<f64>() {
                Ok(n) => Ok(Value::Float(n)),
                Err(_) => Ok(Value::Nil),
            },
            _ => Err(fluentai_core_lib::RuntimeError::host(
                "string->number expects a string",
            )),
        }),
        HostFunction::new("number->string", 1, |args| match &args[0] {
            Value::Float(n) => Ok(Value::String(n.to_string())),
            Value::Integer(n) => Ok(Value::String(n.to_string())),
            _ => Err(fluentai_core_lib::RuntimeError::host(
                "number->string expects a number",
            )),
        }),
    ];
...
pub fn register_list(engine: &RuntimeEngine) -> Result<()> {
    let functions = vec![
        HostFunction::new("list", 0, |args| Ok(Value::List(args.to_vec()))).variadic(),
        HostFunction::new("list-length", 1, |args| match &args[0] {
            Value::List(list) => Ok(Value::Integer(list.len() as i64)),
            _ => Err(fluentai_core_lib::RuntimeError::host(
                "list-length expects a list",
            )),
        }),
        HostFunction::new("list-ref", 2, |args| match (&args[0], &args[1]) {
            (Value::List(list), Value::Integer(n)) => {
                let index = *n as usize;
                if index < list.len() {
                    Ok(list[index].clone())
                } else {
                    Err(fluentai_core_lib::RuntimeError::host(
            ...
            _ => Err(fluentai_core_lib::RuntimeError::host(
                "list-ref expects a list and index",
            )),
        }),
