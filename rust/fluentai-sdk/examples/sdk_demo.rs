...
fn custom_functions_example() -> anyhow::Result<()> {
    ...
    let greet = HostFunction::new("greet", 1, |args| {
        match &args[0] {
            Value::String(name) => {
                Ok(Value::String(std::sync::Arc::new(format!("Hello, {}!", name))))
            }
            _ => Err(fluentai_runtime::RuntimeError::host("greet expects a string")),
        }
    });
    
    let add_tax = HostFunction::new("add-tax", 2, |args| {
        match (&args[0], &args[1]) {
            (Value::Number(amount), Value::Number(rate)) => {
                let tax = amount * (rate / 100.0);
                Ok(Value::Number(amount + tax))
            }
            _ => Err(fluentai_runtime::RuntimeError::host("add-tax expects two numbers")),
        }
    });
...
fn script_execution_example() -> anyhow::Result<()> {
    ...
    // Create a script
    let script = Script::new("fibonacci", r#"
...
fn value_conversion_example() -> anyhow::Result<()> {
    ...
    let get_data = HostFunction::new("get-data", 0, |_| {
        ...
        data.insert("scores".to_string(), Value::List(vec![
            Value::Number(95.0),
            Value::Number(87.0),
            Value::Number(92.0),
        ].into()));
        ...
        let pairs: Vec<Value> = data.into_iter()
