...
fn test_string_concat() {
    ...
    // Test single string
    let result = string_concat.call(&[Value::String("alone".to_string())]).unwrap();
...
fn test_string_upcase_downcase() {
    ...
    assert_eq!(
        downcase.call(&[Value::String("HELLO".to_string())]).unwrap(),
        Value::String("hello".to_string())
    );
    
    assert_eq!(
        upcase.call(&[Value::String("Hello World!".to_string())]).unwrap(),
...
fn test_string_conversions() {
    ...
    // Test string->list
    let result = string_to_list.call(&[Value::String("abc".to_string())]).unwrap();
    match result {
        Value::List(items) => {
            assert_eq!(items.len(), 3);
            assert_eq!(items[0], Value::String("a".to_string()));
    ...
    // Test list->string
    let result = list_to_string.call(&[Value::List(vec![
    ...
    assert_eq!(
        string_to_number.call(&[Value::String("3.14".to_string())]).unwrap(),
        Value::Float(3.14)
    );
    
    assert!(string_to_number.call(&[Value::String("not a number".to_string())]).is_err());
