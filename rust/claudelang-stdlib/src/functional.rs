...
pub fn register(registry: &mut StdlibRegistry) {
    registry.register_all(vec![
        ...
        StdlibFunction::pure("compose", compose, 2, None, "Compose functions right-to-left"),
        ...
        StdlibFunction::pure("partial-right", partial_right, 2, None, "Partial application from right"),
        
        // Currying
        StdlibFunction::pure("curry", curry, 1, Some(1), "Convert function to curried form"),
        StdlibFunction::pure("uncurry", uncurry, 1, Some(1), "Convert curried function to uncurried"),
        
        // Application
        StdlibFunction::pure("apply", apply, 2, Some(2), "Apply function to list of arguments"),
        ...
        StdlibFunction::pure("replicate", replicate, 2, Some(2), "Replicate value n times (alias for repeat)"),
        
        // Grouping
        StdlibFunction::pure("group-by", group_by, 2, Some(2), "Group elements by key function"),
        StdlibFunction::pure("chunk", chunk, 2, Some(2), "Split list into chunks of size n"),
        StdlibFunction::pure("sliding-window", sliding_window, 2, Some(2), "Create sliding windows of size n"),
    ]);
}
...
fn compose(_args: &[Value]) -> Result<Value> {
    ...
    Err(anyhow!("compose: VM integration required for function composition"))
}

fn pipe(_args: &[Value]) -> Result<Value> {
    // This would need VM integration to create a new piped function
    Err(anyhow!("pipe: VM integration required for function composition"))
}
...
fn const_fn(_args: &[Value]) -> Result<Value> {
    ...
    Err(anyhow!("const: VM integration required to create functions"))
}

fn flip(_args: &[Value]) -> Result<Value> {
    // This would need VM integration to create a flipped function
    Err(anyhow!("flip: VM integration required to transform functions"))
}
...
fn partial(_args: &[Value]) -> Result<Value> {
    ...
    Err(anyhow!("partial: VM integration required for partial application"))
}

fn partial_right(_args: &[Value]) -> Result<Value> {
    // This would need VM integration to create a partially applied function
    Err(anyhow!("partial-right: VM integration required for partial application"))
}
...
fn apply(_args: &[Value]) -> Result<Value> {
    ...
    Err(anyhow!("apply: VM integration required for function application"))
}
...
fn fold_right(_args: &[Value]) -> Result<Value> {
    ...
    Err(anyhow!("fold-right: VM integration required for function application"))
}

fn scan(_args: &[Value]) -> Result<Value> {
    // This would need VM integration to call the scanning function
    Err(anyhow!("scan: VM integration required for function application"))
}
...
fn iterate(_args: &[Value]) -> Result<Value> {
    ...
    Err(anyhow!("iterate: VM integration required for function application"))
}
...
fn group_by(_args: &[Value]) -> Result<Value> {
    ...
    Err(anyhow!("group-by: VM integration required for function application"))
}
