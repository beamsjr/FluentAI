...
impl Transformer {
    ...
    fn apply_rule(&self, graph: &mut Graph, rule: &TransformRule) -> Result<bool> {
        ...
        for (node_id, match_result) in transformations {
            let transformed = self.apply_transform(graph, node_id, &rule.transform, &match_result)?;
    ...
    fn apply_transform(
        ...
        match transform {
            ...
            Transform::Conditional { condition, then_transform, else_transform } => {
            ...
            Transform::Extract { name, binding_point } => {
    ...
    fn clone_with_renaming(
        ...
        let node = source.get_node(node_id)
            .ok_or_else(|| MetaprogrammingError::NodeNotFound(node_id))?;
        
        let new_node = match node {
            Node::Variable { name } => {
                Node::Variable {
                    name: var_mapping.get(name).cloned().unwrap_or_else(|| name.clone()),
                }
            }
            Node::Lambda { params, body } => {
                let new_params = params.iter()
                    ...
                    .collect();
                let new_body = self.clone_with_renaming(source, *body, target, id_mapping, var_mapping)?;
                Node::Lambda { params: new_params, body: new_body }
            }
