...
use crate::patterns::{MatchResult, Pattern, PatternMatcher};
...
pub enum Constraint {
    ...
    Sibling { node1: String, node2: String },
    /// Custom predicate
        ...
        match constraint {
            Constraint::Path {
                ...
                if let (Some(&from_id), Some(&to_id)) = (
                    match_result.bindings.get(from),
                    match_result.bindings.get(to),
                ) {
            ...
            Constraint::Ancestor {
                ...
                if let (Some(&anc_id), Some(&desc_id)) = (
                    match_result.bindings.get(ancestor),
                    match_result.bindings.get(descendant),
                ) {
            ...
            Constraint::Sibling { node1, node2 } => {
                if let (Some(&id1), Some(&id2)) = (
                    match_result.bindings.get(node1),
                    match_result.bindings.get(node2),
                ) {
            ...
            Constraint::Predicate { name, args } => self.check_predicate(match_result, name, args),
        }
    }
