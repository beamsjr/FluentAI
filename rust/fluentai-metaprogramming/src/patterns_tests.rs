#[cfg(test)]
mod tests {
    use crate::patterns::{Pattern, NodePattern, MatchResult, PatternMatcher};
    ...
    fn test_node_pattern_variable() {
        let pattern = NodePattern::Variable { name: Some("x".to_string()) };
        match pattern {
    ...
    fn test_node_pattern_equality() {
        let np1 = NodePattern::Variable { name: Some("x".to_string()) };
        let np2 = NodePattern::Variable { name: Some("x".to_string()) };
        let np3 = NodePattern::Variable { name: Some("y".to_string()) };
